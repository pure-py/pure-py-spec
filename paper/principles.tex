...
scope after the \pyinline{match} statement. \PurePy follows: each case is treated as a branch, and the variables
bound by its pattern as assigned. The set of variables a pattern $p$ introduces is $\binds(p)$; the
...
wildcard) may fall through, and the \ruleName{match-partial} rule merges in the empty assignment, as for a
conditional without \pyinline{else}:
...
variables must be distinct, as in Python. The cases must also each be reachable: the \ruleName{cases-cons}
