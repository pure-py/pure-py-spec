...
Well-formedness enforces a small number of principles. This section states each with the programs it
accepts and rejects and the auxiliary functions its rules use; the rules themselves follow. Two analyses
appear throughout: $\assignsS(s)$, the set of variables assigned anywhere in $s$, not descending into nested
...
\emph{capture}. $\capturesS(s)$ is the set of variables of the enclosing scope captured by the closures
within statement $s$, and $\capturesE(e)$ the same for an expression $e$: the free variables of each
