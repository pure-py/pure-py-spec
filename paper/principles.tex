...
\emph{capture}. The set of variables of the enclosing scope captured by the lambdas and nested function
definitions within a statement $s$ is $\capturesS(s)$, with $\capturesE(e)$ the same for an expression $e$: the
free variables of each such function's body, less its parameters and the variables the body assigns, which are local to it. For a
mutual region of function definitions (\secref{syntax}) the names of the functions themselves are also
excluded.
