...
the walrus operator (\pyinline{:=}), which assigns within an expression. \Replaced{A variable is declared once and
assigned at most once on any path (\secref{local-assignment}), so a closure captures a value rather than a mutable
cell, as with Java's requirement that captured locals be effectively final~\cite[\S15.27.2]{gosling2025}. One form
of rebinding remains in the syntax: Python implements a comprehension by updating its generator variables in place
on each iteration, so a lambda within a comprehension may not capture one (\secref{captured-variables}):}{Reassignment of a variable already
