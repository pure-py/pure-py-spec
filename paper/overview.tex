...
\PurePy requires a declaration for every variable, in the form of a bare annotation, and a definite-assignment
analysis tracks when the variable holds a value (\secref{well-formedness}).}
...
the walrus operator (\pyinline{:=}), which assigns within an expression. Reassignment is excluded outright: a
...
result \pyinline{11}. \PurePy rejects the program, since \pyinline{x} is assigned twice, keeping closures over
values rather than mutable cells, as with Java's requirement that captured locals be effectively
final~\cite[\S15.27.2]{gosling2025}. The rules that enforce this are given in \secref{local-assignment}.
