...
the walrus operator (\pyinline{:=}), which assigns within an expression. \Replaced{A Python closure captures a binding rather than a value; since no binding changes, the difference
cannot be observed, a discipline Java imposes on captured locals, which must be effectively
final~\cite[\S15.27.2]{gosling2025}. One binding does change: Python implements a comprehension by updating its
generator variables in place on each iteration, so a lambda within a comprehension may not capture one, since the
updates would otherwise be observable through it (\secref{captured-variables}):}{Reassignment of a variable already
