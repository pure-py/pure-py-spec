...
\replaced{\PurePy requires a declaration for every variable, in the form of a bare annotation, and a definite-assignment
analysis tracks when the variable holds a value (\secref{well-formedness}).}{This motivates the definite-assignment analysis (\secref{well-formedness}).}}
...
the walrus operator (\pyinline{:=}), which assigns within an expression. \Replaced{Reassignment is excluded outright: a
variable is declared once, by an annotation, and assigned at most once on any path, so a closure never sees a
changed value. In Python a closure captures a mutable binding, and a later reassignment is visible through the
closure:}{Reassignment of a variable already
...
result \pyinline{11}. \PurePy rejects the program, \Added{since \pyinline{x} is assigned twice,} keeping closures over
values rather than mutable cells, as with Java's requirement that captured locals be effectively
final~\cite[\S15.27.2]{gosling2025}. The rules that enforce this are given in \replaced{\secref{local-assignment}}{\secref{captured-variables}}.
