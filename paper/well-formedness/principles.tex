...
\Added{The rules for variables serve two aims: that a program has no mutable state (\secref{no-mutable-state}), so a
variable never changes value, and that a well-typed program is also well typed under mypy. Within these limits
the rules permit as much as possible. For the first aim, a variable is introduced by a declaration, parameter,
import, pattern or generator, and receives its value at most once, with its declaration or by later assignment;
an assignment never introduces a variable and never changes one already assigned. Python rebinds a generator
...
condition of \ruleName{list-comp}, \ruleName{dict-comp} and \ruleName{qual-generator}, illustrated in
