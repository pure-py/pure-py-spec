...
Python has no variable declarations: a variable is introduced by assigning to it, and a local variable is
in scope throughout the function body, wherever the assignment occurs. \PurePy requires a declaration, in the
form of a bare annotation such as \pyinline{x: int}, which Python has allowed since version 3.6, and separates
...
already. The \ruleName{declare} rule binds the variable at $\FalseTy{\tau}$, declared at $\tau$ but not
definitely assigned; the \ruleName{assign} rule requires that entry, checks the expression against $\tau$ and
...
assigns it, otherwise leaving it at $\False$ (\secref{well-formedness-statements}). A variable is thus
declared once in its scope and assigned at most once on any path, as a blank final in
Java~\cite[\S16]{gosling2025}. Branch-local variables are therefore permitted, but using one after the
...
of the same name is rejected even in another branch of a conditional, as is a declaration of a parameter or
of an imported name.

Python's function-level scoping is kept as well. The \ruleName{def} rule introduces every variable declared
or assigned by the body, $\assignsS(s)$, at $\False$ before checking the body, so a reference before the
...
In Python a closure holds a mutable binding, so a variable captured by a lambda or nested function
definition may be reassigned afterwards, changing the value seen by the function (\secref{no-mutable-state}).
Single assignment excludes this: a closure can capture only a definitely assigned variable, which no later
statement may assign. One case remains, in comprehensions: in Python a comprehension mutates its generator
...
Here $\capturesS(\seq{g})$ is the set of variables of the enclosing scope captured by the lambdas within the
qualifiers $\seq{g}$ of a comprehension, with $\capturesE(e)$ computing the same for an expression $e$: the
