...
The \Replaced{\ruleName{def-body}}{\ruleName{def}} rule introduces every variable declared or assigned by the body, $\assignsS(s)$, at $\Unbound$
...
dependency static: a \emph{mutual region}, \Replaced{a maximal contiguous block of \pyinline{def} statements}{a maximal run of consecutive \pyinline{def} statements in a sequence} (\figref{syntax-stmt}), is a single simultaneous binding. The \ruleName{def} rule checks each body in a
