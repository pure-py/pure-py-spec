...
only where an assignment covers every path to the reference. The \ruleName{wf-var} rule requires the variable
to be definitely assigned, $\Gamma(x) = \statusDefAssigned$; the \ruleName{wf-assign} rule makes it so, with
$x = e$ assigning $\set{\bind{x}{\statusDefAssigned}}$; and the \ruleName{wf-if} rules combine the branches with
...
Python's function-level scoping is kept as well. The \ruleName{wf-def} rule introduces every variable the
...
by that statement itself. The \ruleName{wf-seq} rule imposes the first as a side condition on a sequence
...
\ruleName{wf-list-comp}, \ruleName{wf-dict-comp} and \ruleName{wf-qual-generator}. For instance
...
themselves are also excluded, since the \ruleName{wf-def} rule binds them in every body, consistent with
...
sequence (\figref{syntax-stmt}), is a single simultaneous binding. The \ruleName{wf-def} rule checks each body in a
...
\noindent This is rejected statically: \pyinline{even} forms a mutual region of its own in which \pyinline{odd}
is not yet bound, so the reference fails the \ruleName{wf-var} rule. Python accepts the program, since the
...
\ruleName{wf-match} rule checks each body in a context in which they are definitely assigned, and combines
the cases with $\merge$ as the \ruleName{wf-if} rules do, so a variable is definitely assigned after the match
only if every case binds it or returns. A match whose last case is not a catch-all (i.e.~a variable or
wildcard) may fall through; in this case the \ruleName{wf-match-partial} rule merges in the empty assignment set, as for a
...
variables must be distinct, as in Python. Every case must also be reachable: the \ruleName{wf-cases-cons}
...
sequence returns only if its last statement returns and every earlier one assigns: the \ruleName{wf-seq}
...
statement runs. Loading is checked statically: the \ruleName{wf-module} rule checks a body and computes its
signature, and the \ruleName{wf-import} and \ruleName{wf-from-import} rules statically load each target and its
ancestors first, so an import cycle has no derivation. A module may not bind a name that is also the name of one of its
submodules, a side condition of \ruleName{wf-module}, since Python resolves the clash by load order. A
qualified reference must resolve through the referring module's own imports to a definitely assigned
member (\ruleName{wf-attr-module}). A submodule is not an attribute of its parent until it is loaded, so
