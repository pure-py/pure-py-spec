...
comprehension mutates its generator variables on each iteration, so a lambda in its body or in a later
qualifier may not capture one, a side condition of \ruleName{list-comp}, \ruleName{dict-comp} and
\ruleName{qual-generator}: \pyinline{[lambda x: x**i for i in range(10)]} is rejected, since under Python
every element would compute \pyinline{x**9}. A dataclass declaration counts as an assignment of its class name here, although re-declaring a class name is
