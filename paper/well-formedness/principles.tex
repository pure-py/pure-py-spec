\subsection[Key concepts]{\Replaced{Key concepts}{Design principles}}
\label{sec:principles}

\Replaced{This section introduces the concepts underlying the rules, each illustrated by admitted and rejected
programs; the rules follow in \secref{expression-typing} to \secref{well-formedness-modules}.}{Well-formedness
enforces several constraints, summarised here with the programs each accepts and rejects; the rules follow in
\secref{well-formedness-statements} and \secref{well-formedness-modules}.} \Added{The rules for
variables serve the two aims of \secref{no-mutable-state} and \secref{python-compatibility}, no mutable state and
compatibility with mypy, and within these limits permit as much as possible.}
...
already, the rule applied by Java to blank finals~\cite[\S16]{gosling2025}. The \ruleName{declare} rule binds the variable at $\DU{\tau}$, declared at $\tau$ and
...
The \ruleName{def} rule introduces every variable declared or assigned by the body, $\assignsS(s)$, at $\Unbound$
...
\subsubsection[Reachability]{\Replaced{Reachability}{No unreachable code}}
