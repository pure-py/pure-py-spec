...
\input{spec/well-formedness/variable-aims}
\Added{Python rebinds a generator
variable on each iteration, so \PurePy forbids a lambda in a comprehension from capturing such a variable, since
otherwise it would see a later value.}

\Added{The second aim has consequences for scope. A notion of scope usually determines the parts of a
...
is rejected as unbound rather than referring to the enclosing variable. mypy agrees, with one exception: at
module level a use before the declaration of a builtin name refers to the builtin, as it does when Python runs
the module. \PurePy treats the builtins as an enclosing scope like any other, and therefore rejects a use of a builtin
name if the module later declares that name. A pattern introduces its variables rather than assigning them, so a pattern
...
Python has no variable declarations: a variable is introduced by assigning to it, and a local variable is
in scope throughout the function body, wherever the assignment occurs. \PurePy requires a variable declaration, a type annotation such as \pyinline{x: int}, which Python has allowed since version 3.6, and separates
...
returns, and is otherwise possibly unassigned, $\PU{\tau}$ (\secref{well-formedness-statements}). A variable is thus
...
A module body is a scope in the same way, with the builtins as its enclosing scope, so a declaration in the body
hides a builtin throughout the module. Here \PurePy is stricter than mypy, which resolves a use before the
declaration to the builtin, following Python's run-time lookup:
...
variables on each iteration, so \PurePy forbids a lambda in the body or in a later qualifier from capturing one, a
side condition of \ruleName{list-comp}, \ruleName{dict-comp} and \ruleName{qual-generator}, illustrated in
\secref{no-mutable-state}.
