...
Scientific computing in Python rests on libraries such as NumPy and Pandas, whose interfaces depend on
...
updating in place, is an example of such an interface.
...
checker can read. The lightest option is a decorator: with effects the default, \pyinline{@pure} marks functions in
...
The guarantee of \secref{python-compatibility} is informal. To prove the guarantee we would need a reference semantics
of Python against which to state it; one candidate is the core calculus of Politz et al.~\cite{politz2013}, which
