...
\subsection{Agreement with a reference semantics of Python}
\label{sec:agreement}

The guarantee of \secref{python-compatibility} is stated against Python itself: on every run it defines,
\PurePy produces Python's result. A proof needs a reference semantics of Python to state it against. Of the
formalisations in \secref{related}, the closest is the core calculus of Politz et al.~\cite{politz2013},
which covers a subset of Python, so the first question is whether that subset contains \PurePy; a proof
would then show that, for every well-formed program, our evaluation judgement and the calculus agree on the
result.
