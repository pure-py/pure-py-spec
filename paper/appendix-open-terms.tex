...
(see \secref{local-assignment}). However, some forms of late binding must be allowed, as we could otherwise not
...
that are used within a closure (see \secref{local-assignment}), and an \emph{environment} $\Gamma$,
...
\secref{local-assignment}). If the variable is not bound in the current environment, it is typed at the
