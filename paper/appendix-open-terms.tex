...
(see \secref{captured-variables}). However, some forms of late binding must be allowed, as we could otherwise not
...
that are used within a closure (see \secref{captured-variables}), and an \emph{environment} $\Gamma$,
...
\secref{captured-variables}). If the variable is not bound in the current environment, it is typed at the
