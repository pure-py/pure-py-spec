...
(see \replaced{\secref{local-assignment}}{\secref{captured-variables}}). However, some forms of late binding must be allowed, as we could otherwise not
...
that are used within a closure (see \replaced{\secref{local-assignment}}{\secref{captured-variables}}), and an \emph{environment} $\Gamma$,
...
the stack. For variables, there are three different cases. In $\ruleName{open-var1}$, the variable is
...
\replaced{\secref{local-assignment}}{\secref{captured-variables}}). If the variable is not bound in the current environment, it is typed at the
surrounding scope as shown in $\ruleName{open-var2}$. The base-case is illustrated by $\ruleName{open-var3}$, where a variable
...
For statements, the $\ruleName{open-pass}$ is trivial. For an assignment, the assigned variable cannot be part of the set of
...
The rules for conditional evaluation ($\ruleName{open-if1}$ and $\ruleName{open-if2}$) follow a similar structure to the ones for
...
The rule for function definition $\ruleName{open-def}$ works similarly to the one for lambda expressions, with the added step of
...
\input{paper/fig/open-term-typing}
