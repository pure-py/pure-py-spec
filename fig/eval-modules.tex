...
{
  x \notin \dom(\mu) \\
  \mu, \rho^0(x), \mathcal{M}(x) \Rightarrow \mu', \rho
}
{
  \mu, x \Rightarrow_{\mathsf{load}} \mu' \runion \set{\bind{x}{\rho^0(x) \runion \rho}}
}
\and
\inferrule*[lab={\ruleName{load-qualified}}]
{
  q.x \notin \dom(\mu) \\
  \mu, q \Rightarrow_{\mathsf{load}} \mu' \\
  \mu', \rho^0(q.x), \mathcal{M}(q.x) \Rightarrow \mu'', \rho
}
{
  \mu, q.x \Rightarrow_{\mathsf{load}} \mu'' \runion \set{\bind{q.x}{\rho^0(q.x) \runion \rho}}
}
\end{mathpar}
