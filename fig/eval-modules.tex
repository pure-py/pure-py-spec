...
{
  \mu, \rho \runion \set{c \mapsto \clsEntry{\Gamma}{q}{\ownFields(\phi)}{\bot}}, m \Rightarrow \mu', \rho'
}
{
  \mu, \rho, \exDataclass{c}{\phi} \cons m \Rightarrow \mu', \set{c \mapsto \clsEntry{\Gamma}{q}{\ownFields(\phi)}{\bot}} \runion \rho'
}
\and
\inferrule*[lab={\ruleName{eval-class-extend}}]
{
  \mu, \rho \runion \set{c \mapsto \clsEntry{\Gamma}{q}{\ownFields(\phi)}{c'}}, m \Rightarrow \mu', \rho'
}
{
  \mu, \rho, \exDataclassExtend{c}{c'}{\phi} \cons m \Rightarrow \mu', \set{c \mapsto \clsEntry{\Gamma}{q}{\ownFields(\phi)}{c'}} \runion \rho'
}
\end{mathpar}
...
{
  \mu, x \Rightarrow_{\mathsf{load}} \mu' \runion \set{x \mapsto \exObj{\Gamma_x}{\bot}{\set{\text{\pyinline{__name__}} \mapsto \rho_x^0(\text{\pyinline{__name__}})} \runion \rho}}
}
\and
...
{
  \mu, q.x \Rightarrow_{\mathsf{load}} \mu'' \runion \set{q.x \mapsto \exObj{\Gamma_{q.x}}{\bot}{\set{\text{\pyinline{__name__}} \mapsto \rho_{q.x}^0(\text{\pyinline{__name__}})} \runion \rho}}
}
\end{mathpar}
