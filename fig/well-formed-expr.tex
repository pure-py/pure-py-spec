\begin{figure}
\begin{subfigure}{\textwidth}
\flushleft \shadebox{$\Gamma \vdash q : \theta$}
\begin{mathpar}
\small
\inferrule*[lab={\ruleName{simple-var}}]
{
  \bind{x}{\statusDefAssigned} \in \Gamma
}
{
  \Gamma \vdash x : \statusDefAssigned
}
\and
\inferrule*[lab={\ruleName{simple-module}}]
{
  \bind{x}{\modLoaded{q}{\Gamma'}} \in \Gamma
}
{
  \Gamma \vdash x : \modLoaded{q}{\Gamma'}
}
\and
\inferrule*[lab={\ruleName{simple-class}}]
{
  \bind{c}{C} \in \Gamma
}
{
  \Gamma \vdash c : C
}
\and
\inferrule*[lab={\ruleName{qualified}}]
{
  \Gamma \vdash q : \modLoaded{q'}{\Gamma'} \\
  \Gamma' \vdash x : \theta
}
{
  \Gamma \vdash q.x : \theta
}

\end{mathpar}
\subcaption{Name $q$ resolves to context entry $\theta$ under $\Gamma$}
\end{subfigure}

\vspace{2mm}

\begin{subfigure}{\textwidth}
\flushleft \shadebox{$\Gamma \vdash e$}
\begin{mathpar}
\small
\inferrule*[lab={\ruleName{var}}]
{
  \bind{x}{\statusDefAssigned} \in \Gamma
}
{
  \Gamma \vdash x
}
\and
\inferrule*[lab={\ruleName{var-builtin}}]
{
  x \notin \dom(\Gamma) \\
  \added{\mathcal{P}(\pyinline{builtins}) = (\rho, \Gamma')} \\
  \bind{x}{\statusDefAssigned} \in \replaced{\Gamma'}{\Gamma_{\pymath{builtins}}\ }
}
{
  \Gamma \vdash x
}
\and
\inferrule*[lab={\ruleName{literal}}]
{
  \strut
}
{
  \Gamma \vdash \ell
}
\and
\inferrule*[lab={\ruleName{lambda}}]
{
  \Gamma \override \statusDefAssigned(\seq{x}) \vdash e
}
{
  \Gamma \vdash \exLambda{\seq{x}}{e}
}
\and
\inferrule*[lab={\ruleName{call}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e_i \quad (\forall i)
}
{
  \Gamma \vdash \exCall{e}{e_1, \ldots, e_n}
}
\and
\inferrule*[lab={\ruleName{constr}}]
{
  \Gamma \vdash q : C \\
  \deleted{\fields(C) = x_1, \ldots, x_n \cons \seq{x}\,'} \\
  \deleted{\set{\seq{y}} = \set{\seq{x}\,'}} \\
  \replaced{\Gamma \vdash \fieldMap(C, \seq{e})(x) \quad (\forall x \in \fields(C))}{\Gamma \vdash e_i \quad (\forall i)\ }
}
{
  \Gamma \vdash \exConstr{q}{\replaced{\seq{e}}{e_1, \ldots, e_n, y_1 \mathord{=} e_{n+1}, \ldots, y_m \mathord{=} e_{n+m}\ }}
}
\and
\inferrule*[lab={\ruleName{attr-module}}]
{
  \Gamma \vdash \exAttr{e}{x} : \statusDefAssigned
}
{
  \Gamma \vdash \exAttr{e}{x}
}
\and
\inferrule*[lab={\ruleName{attr-object}}]
{
  \Gamma \vdash e
}
{
  \Gamma \vdash \exAttr{e}{x}
}
\and
\inferrule*[lab={\ruleName{dict}}]
{
  \Gamma \vdash e_i \quad (\forall i) \\
  \Gamma \vdash e_i' \quad (\forall i)
}
{
  \Gamma \vdash \exDict{\bind{e_1}{e_1'}, \ldots, \bind{e_n}{e_n'}}
}
\and
\inferrule*[lab={\ruleName{list}}]
{
  \Gamma \vdash e_i \quad (\forall i)
}
{
  \Gamma \vdash \exList{e_1, \ldots, e_n}
}
\and
\inferrule*[lab={\ruleName{tuple}}]
{
  \Gamma \vdash e_i \quad (\forall i)
}
{
  \Gamma \vdash \exTuple{e_1, \ldots, e_n}
}
\and
\inferrule*[lab={\ruleName{subscript}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e'
}
{
  \Gamma \vdash \exSubscript{e}{e'}
}
\and
\inferrule*[lab={\ruleName{binop}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e'
}
{
  \Gamma \vdash \exBinOp{e}{\odot}{e'}
}
\and
\inferrule*[lab={\ruleName{unary-op}}]
{
  \Gamma \vdash e
}
{
  \Gamma \vdash \exUnaryOp{\ominus}{e}
}
\and
\inferrule*[lab={\ruleName{and}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e'
}
{
  \Gamma \vdash \exAnd{e}{e'}
}
\and
\inferrule*[lab={\ruleName{or}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e'
}
{
  \Gamma \vdash \exOr{e}{e'}
}
\and
\inferrule*[lab={\ruleName{cond}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash e_1 \\
  \Gamma \vdash e_2
}
{
  \Gamma \vdash \exCondExpr{e_1}{e}{e_2}
}
\and
\inferrule*[lab={\ruleName{list-comp}}]
{
  \Gamma \vdash \seq{g} : \Delta \\
  \Gamma \override \Delta \vdash e
}
{
  \Gamma \vdash \exListComp{e}{g_1\;\ldots\;g_n}
}
\and
\inferrule*[lab={\ruleName{dict-comp}}]
{
  \Gamma \vdash \seq{g} : \Delta \\
  \Gamma \override \Delta \vdash e \\
  \Gamma \override \Delta \vdash e'
}
{
  \Gamma \vdash \exDictComp{e}{e'}{g_1\;\ldots\;g_n}
}
\end{mathpar}
\subcaption{Expression $e$ is well-formed under $\Gamma$}
\end{subfigure}

\vspace{2mm}

\begin{subfigure}{\textwidth}
\flushleft \shadebox{$\Gamma \vdash \seq{g} : \Delta$}
\begin{mathpar}
\small
\inferrule*[lab={\ruleName{quals-nil}}]
{
  \strut
}
{
  \Gamma \vdash \varepsilon : \varnothing
}
\and
\inferrule*[lab={\ruleName{qual-guard}}]
{
  \Gamma \vdash e \\
  \Gamma \vdash \seq{g} : \Delta
}
{
  \Gamma \vdash \qualGuard{e} \cons \seq{g} : \Delta
}
\and
\inferrule*[lab={\ruleName{qual-generator}}]
{
  \Gamma \vdash e \\
  \Gamma \override \set{\bind{x}{\statusDefAssigned}} \vdash \seq{g} : \Delta
}
{
  \Gamma \vdash \qualGenerator{x}{e} \cons \seq{g} : \set{\bind{x}{\statusDefAssigned}} \override \Delta
}
\end{mathpar}
\subcaption{Comprehension qualifiers $\seq{g}$ bind $\Delta$ under $\Gamma$}
\end{subfigure}
\caption{Expression well-formedness}
\label{fig:well-formed-expr}
\end{figure}
