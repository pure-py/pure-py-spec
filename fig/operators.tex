...
\pyinline{!=} & $\val{\pyinline{True}}$ if both operands are \pyinline{nan} or $\eqOp(v, v') = \pyinline{False}$, and $\val{\pyinline{False}}$ \replaced{if $\eqOp(v, v') = \pyinline{True}$}{otherwise\ } \\
\pyinline{in} & $\val{\memOp(v', v)}$ \\
\pyinline{not in} & $\val{\pyinline{True}}$ if $\memOp(v', v) = \pyinline{False}$, and $\val{\pyinline{False}}$ \replaced{if $\memOp(v', v) = \pyinline{True}$}{otherwise\ } \\
