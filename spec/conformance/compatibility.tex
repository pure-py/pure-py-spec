...
statically\paperonly{ (\secref{no-coercions})}\speconly{ (summarised in the informative \annexref{exclusions})}. An implementation may abort the run at a dynamically excluded operation, but is not obliged to. Agreement with Python is guaranteed only for
valid runs; on any other run a stock Python interpreter simply produces the Python result. \added{An implementation conforms if it accepts
