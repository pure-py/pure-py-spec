...
$(\Gamma \override \Delta)(x) = \Gamma(x) \override \Delta(x)$, where on entries
\begin{defalign}
...
$(\Gamma \merge \Delta)(x) = \Gamma(x) \merge \Delta(x)$, where on definite-assignment statuses $a, a' \in \B$
\begin{defalign}
\bot \merge \bot &= \bot \\
...
\label{def:extend}
$(\Gamma \extend \Delta)(x) = \Gamma(x) \extend \Delta(x)$, where on entries
\begin{defalign}
