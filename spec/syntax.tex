...
Pointwise, $(\Gamma \override \Delta)(x) = \Gamma(x) \override \Delta(x)$, where on entries
\begin{defalign}
...
Pointwise, $(\Gamma \merge \Delta)(x) = \Gamma(x) \merge \Delta(x)$, where on definite-assignment statuses $a, a' \in \B$
\begin{defalign}
\bot \merge \bot &= \bot \\
...
\label{def:extend}
Pointwise, $(\Gamma \extend \Delta)(x) = \Gamma(x) \extend \Delta(x)$, where on entries
\begin{defalign}
...
\noindent Context extension combines module references for the same module recursively, preferring loaded references to stubs, and is otherwise right-biased like override. It is used when processing import prefixes, where two imports rooted at the same module must combine rather than shadow.
