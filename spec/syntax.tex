...
juxtaposition, $s\;s'$; a function body, a branch of a conditional, the body of a case and the body of a module after its
imports are statements. A \emph{mutual region} $d_1 \ldots d_{n+1}$ is a maximal
...
$\seq{\iota}$ followed by a statement $s$; since imports are not statements, they can appear neither
within a statement (and hence not in function bodies) nor after it. The statement begins with the
assignment $\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}$, which the encoding of a source tree
supplies, modelling the $\pymath{\_\_name\_\_}$ binding the runtime supplies automatically.
...
$\dom(\mathcal{M}) \cup \dom(\mathcal{P})$.\footnote{A source tree encodes such an $\mathcal{M}$: each module's body is the statements of its
\texttt{.py} file, or of its \texttt{\_\_init\_\_.py} for a package, preceded by the assignment
$\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}$; a package directory without an \texttt{\_\_init\_\_.py}
(a namespace package, PEP 420, \url{https://peps.python.org/pep-0420/}) encodes that assignment alone.
Prefix-closure thus holds by construction.} The maps $\mathcal{M}$ and $\mathcal{P}$ are fixed and ambient in all
