...
$s$ & $::=$ & $\exPass$ & (Pass) \\
    & $\mid$ & $s\;s'$ & (Seq) \\
    & $\mid$ & \added{$x\exAnnot{\psi}$} & \added{(Declare)} \\
    & $\mid$ & $\exAssign{x}{e}$ & (Assign) \\
    & $\mid$ & $\exAssign{x\exAnnot{\psi}}{e}$ & \replaced{(DeclareAssign)}{(AnnAssign)} \\
    & $\mid$ & $e$ & (ExprStmt) \\
    ...
    & $\mid$ & \replaced{$\exMatch{e}{b_1 \;\ldots\; b_{n+1}}$}{$\exMatch{e}{h_1 \;\ldots\; h_{n+1}}$} & (Match) \\
...
$d$ & $::=$ & $\exDef{f}{\bind{x_1}{\psi_1}, \ldots, \bind{x_n}{\psi_n}}{\exRetAnnot{\psi}}{s}$ & (Def)
\end{tabular}
\end{center}
