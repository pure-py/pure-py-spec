\begin{figure}
\small
\begin{center}
\begin{tabular}{rcll}
\grammarGroup{Expressions} \\
$e$ & $::=$ & $x$ & (Var) \\
    & $\mid$ & $\ell$ & (Literal) \\
    & $\mid$ & $\exCall{e}{e_1, \ldots, e_n}$ & (Call) \\
    & $\mid$ & \replaced{$\exConstr{q[\seq{\psi}]}{a}$}{$\exConstr{q}{a}$} & (Constr) \\
    & $\mid$ & $\exDict{\bind{e_1}{e_1'}, \ldots, \bind{e_n}{e_n'}}$ & (Dict) \\
    & $\mid$ & $\exAttr{e}{x}$ & (Attribute) \\
    & $\mid$ & $\exSubscript{e}{e'}$ & (Subscript) \\
    & $\mid$ & $\exBinOp{e}{\odot}{e'}$ & (BinOp) \\
    & $\mid$ & $\exUnOp{\ominus}{e}$ & (UnOp) \\
    & $\mid$ & $\exAnd{e}{e'}$ & (And) \\
    & $\mid$ & $\exOr{e}{e'}$ & (Or) \\
    & $\mid$ & $\exCondExpr{e_1}{e}{e_2}$ & (CondExpr) \\
    & $\mid$ & $\exList{e_1, \ldots, e_n}$ & (List) \\
    & $\mid$ & $\exTuple{e_1, \ldots, e_n}$ & (Tuple) \\
    & $\mid$ & $\exListComp{e}{g_1\;\ldots\;g_n}$ & (ListComp) \\
    & $\mid$ & $\exDictComp{e}{e'}{g_1\;\ldots\;g_n}$ & (DictComp) \\
    & $\mid$ & $\exLambda{x_1, \ldots, x_n}{e}$ & (Lambda)
   \\[3mm]
\grammarGroup{Comprehension qualifiers} \\
$g$ & $::=$ & $\qualGuard{e}$ & (Guard) \\
    & $\mid$ & $\qualGenerator{x}{e}$ & (Generator)
   \\[3mm]
\grammarGroup{Literals} \\
$\ell$ & $::=$ & $n$ & (Number) \\
    & $\mid$ & $w$ & (String) \\
    & $\mid$ & $\pyinline{True}$ & (True) \\
    & $\mid$ & $\pyinline{False}$ & (False) \\
    & $\mid$ & $\pyinline{None}$ & (None)
   \\[3mm]
\grammarGroup{Argument lists} \\
$a$ & $::=$ & $u_1, \ldots, u_n, x_{n+1} \mathord{=} u_{n+1}, \ldots, x_{n+m} \mathord{=} u_{n+m}$ & (Args)
   \\[3mm]
\grammarGroup{Patterns} \\
$p$ & $::=$ & $\ell$ & (PatLiteral) \\
    & $\mid$ & $-n$ & (PatNegLit) \\
    & $\mid$ & $x$ & (PatVar) \\
    & $\mid$ & $\patWild$ & (PatWildcard) \\
    & $\mid$ & $\patList{p_1, \ldots, p_n}$ & (PatList) \\
    & $\mid$ & $\patTuple{p_1, \ldots, p_n}$ & (PatTuple) \\
    & $\mid$ & $\patConstr{q}{a}$ & (PatConstr) \\
    & $\mid$ & $\patDict{\bind{w_1}{p_1}, \ldots, \bind{w_n}{p_n}}$ & (PatDict) \\
    & $\mid$ & $p\;\pyinline{as}\;x$ & (PatAs)
\end{tabular}
\end{center}
\caption{Expressions and patterns}
\label{fig:syntax-expr}
\end{figure}
