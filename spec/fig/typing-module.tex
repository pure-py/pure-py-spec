...
\caption{Name $x$ imported from module signature $\Gamma$ has context entry $\theta$}
...
\inferrule*[lab={\ruleName{from-import}}]
{
  ...
  \replaced{\set{q^\dagger_1, \ldots, q^\dagger_m} = \set{q^\dagger \properPrefixOf q' \mid q^\dagger \not\prefixOf q}}{q^\dagger_1 \properPrefixOf \cdots \properPrefixOf q^\dagger_m \text{ the } q^\dagger \properPrefixOf q' \text{ with } q^\dagger \not\prefixOf q} \\
  \added{q^\dagger_1 \properPrefixOf \cdots \properPrefixOf q^\dagger_m} \\
...
{
  \Sigma \vdash_{\mathsf{load}} q : \Gamma' \binding \replaced{\Sigma^\dagger}{\Sigma_1} \\
  \replaced{\Sigma^\dagger}{\Sigma_1} \vdash q, \modLoaded{q}{\Gamma' \extend \set{\bind{x}{\theta}}} \loadsAs \theta' \binding \Sigma'
}
{
  \Sigma \vdash q.x, \theta \loadsAs \theta' \binding \Sigma'
}
\end{mathpar}
\subcaption{Reference $\theta$ for module $q$ loads as reference $\theta'$ for root of $q$, statically loading the modules containing $q$}
\end{subfigure}
...
{
  \mathcal{P}(q) = (\replaced{\Gamma}{\Gamma'}, \rho)
}
{
  \Sigma \vdash_{\mathsf{load}} q : \replaced{\Gamma}{\Gamma'} \binding \Sigma
}
\and
\inferrule*[lab={\ruleName{module}}]
{
  \mathcal{M}(q) = \seq{\iota}\;t \\
  \mathcal{P}(\pyinline{builtins}) = (\replaced{\Gamma}{\Gamma'}, \rho) \\
  \Sigma; q \vdash \seq{\iota} : \replaced{\Gamma'}{\Gamma} \binding \replaced{\Sigma^\dagger}{\Sigma_1} \\
  \replaced{\Sigma^\dagger}{\Sigma_1}; q; \replaced{\Gamma}{\Gamma'} \override \replaced{\Gamma'}{\Gamma} \mathbin{\added{\override}} \added{\set{\bind{x}{\Unbound} \mid x \in \assignsS(t)}} \vdash t : \Assigns{\Delta} \binding \Sigma' \\
  \added{\wellScoped(\set{\declaresS(\seq{\iota})}, t)} \\
  (\dom(\replaced{\Gamma'}{\Gamma}) \cup \dom(\Delta)) \cap \dom(\submods(q)) = \varnothing
}
{
