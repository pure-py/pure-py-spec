...
$\rho, p, v \matches m$\speconly{ (\figref{match})}. A \emph{match result} $m$ is either $\matchOk{\rho'}$, a match that binds
the pattern's variables to the environment $\rho'$, or $\noMatch$, a failure to match. Where a pattern
and a value are not comparable, as a list pattern against a tuple, there is no derivation: \PurePy can say
