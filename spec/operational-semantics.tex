...
$\noMatch$, a failure to match. A \pyinline{match} statement takes the first case whose pattern matches
the value, running its body under the bindings, and does nothing if no case matches; the auxiliary
function $\dispatch$ (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:dispatch}}) selects that
case.
\paperinput{paper/operational-semantics/match}
Having no derivation is a third
possibility, distinct from both match results: the match has no meaning, and \PurePy gives the run no result. A pattern and a
