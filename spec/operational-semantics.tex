...
\input{spec/fig/well-formed-env}
...
before the failing one was reached\speconly{ (\figref{eval-fails})}\paperonly{ (\figref{eval-example})}. The statement, sequence and module judgements use
outcomes of the same form, carrying a result, a sequence of values or an environment in place of a value.
\paperinput{paper/evaluation-example}
...
\paperinput{paper/operational-values}
...
is given in \speconly{\figrefTwo{eval-block-outcome}{eval-block-fails}}\paperonly{full in \specification}. Function definitions delimit
...
Expression evaluation $\rho, e \Rightarrow o$\speconly{ (\figrefTwo{eval-outcome}{eval-outcome-ctd})}, for any $\rho : \Gamma$ and any $e$ well-formed in $\Gamma$, yields an
