...
$\dispatch$ (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:dispatch}}) selects that case.
\paperinput{paper/operational-semantics/match}%
Whether a pattern matches is the pattern-matching judgement $\rho, p, v \matches m$ (\figref{match}), a
deterministic partial function. A \emph{match result} $m$ is either $\matchOk{\rho'}$, a match binding
$\rho'$, or $\noMatch$, a failure to match. Having no derivation is a third possibility, distinct from
both: where a pattern and a value are not comparable, as a list pattern against a tuple, \PurePy can say
neither that they match nor that they do not, and gives the run no result. Sub-results compose with
...
\noindent with $\matchOk{\varnothing}$ as left and right unit. A constructor pattern matches any object
whose class has the pattern's class among its ancestors, and its sub-patterns bind the fields of the
pattern's class. An object records its class entry, whose qualified name is the identity of the class:
...
Statement evaluation $\rho, s \Rightarrow r$ is given in \speconly{\figrefTwo{eval-block-outcome}{eval-block-fails}}\paperonly{full in \specification}. Function definitions delimit
