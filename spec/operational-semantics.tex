...
\figref{eval-imports}.}\paperonly{Module and program evaluation are given in full in \specification.} Context extension (\figref{context-ops}) applies to environments as well, with the same equations on values in place of entries: a loaded module value $\modLoaded{q}{\rho}$ carries an environment where the entry $\modLoaded{q}{\Gamma}$ carries a context, and any other value is overridden as an entry is. The loading judgement $q \Rightarrow_{\mathsf{load}} o$ evaluates the import prefix
