...
values of type $\tau$ whose outermost head is not among those in $H$. A head itself denotes values: a
...
$\Sigma \vdash k : \tau$ checks each excluded head and each component of a shape
...
$\shapesOf(\Sigma, \tau, H)$: one shape per member of a union, and none at all when $H$ exhausts $\tau$. No shape carries a
...
A pattern matches a shape of the same form directly, or splits it first (\figref{split}): splitting takes the head
...
(\figrefTwo{pat}{pat-seq}). A pattern has no derivation against a shape that it cannot match, so
