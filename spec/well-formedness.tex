...
join of the results (\ruleName{subscript-union}, \ruleName{attr-union}, \ruleName{call-union}); iteration needs no
such rule, because $\elemTy$ takes a union to the join of its members' element types.
\citet{dunfield2004} give a single union-elimination rule, stated for an arbitrary evaluation context;
with only four elimination forms, a rule for each is simpler than introducing evaluation contexts.
