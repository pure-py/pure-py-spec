...
judgement $\Sigma; \Gamma; \zeta; \sigma \vdash \seq{h} : S, L$ (\figref{typing-cases}), which matches each
case's pattern in turn against the values of $\sigma$ which have not been matched by the earlier cases,
checks the body under the pattern's bindings, and merges the outcomes into $S$; the
\emph{residual} $L$ is a static over-approximation of the values which are not matched by any of the cases
(\secref{well-formedness-patterns}). Each pattern must agree with $\sigma$ (\figref{agrees}). The
...
The \ruleName{cases} rule (\figref{typing-cases}) takes the cases in the order $\dispatch$ tries them at run
