...
which matches each case's pattern in turn against the values of
$\sigma$ which have not been matched by the earlier cases, yielding each pattern's bindings $\Delta_i$ and the
\emph{residual} $L$, a static over-approximation of the values which are not matched by any of the cases
(\secref{well-formedness-patterns}). The body of each case is
...
To check a \pyinline{match}, each case is matched in turn against the residual: those parts of the
...
The matching judgement $\Sigma; \Gamma \vdash L, p \matches K, L' \binding \Delta$ divides an ordered set of shapes into the
matched shapes $K$ and the residual $L'$, binding $\Delta$ (\figrefTwo{pat}{pat-seq}). It is defined from a
judgement on a single shape, which for a tuple, list, dictionary or constructor shape matches the sub-patterns
against the component shapes position by position. A pattern has no derivation against a shape that it cannot match, so
