...
\ruleName{module} require the declarations of a scope, together with its parameters or its imported names,
to be distinct and none of them to be bound by a pattern in the scope. A name therefore cannot be declared
twice in a scope, whether on one path or in different branches, nor can a declaration share its name with a
parameter, an imported name or a pattern variable of the same scope. Each of these cases is rejected by mypy as
a redefinition. The body of a lambda or nested definition is checked under the context in force where it is defined,
...
prefers a locally \Replaced{bound}{assigned} name to one visible in an enclosing scope. \Added{As for any scope, the parameters together with the names declared in the body must be distinct and not
bound by a pattern in the body. The function names of the mutual region are not among these; they are
...
\pyinline{builtins} are in scope unqualified, a name bound by the body shadows a builtin throughout, and an
imported name cannot be assigned by the body. The names declared by the body must be distinct, and none may be
an imported name or a name bound by a pattern.}{overridden by the imported context, so the members of \pyinline{builtins} are in scope
unqualified.}
