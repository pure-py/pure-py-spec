...
ordered set of \emph{shapes} (\figref{shapes}) drawn from the scrutinee's type and covering every value not matched by any of the
earlier cases. Matching the case removes from the residual the values matched
by $p$ (\figref{pat}); a case that matches no shape of the residual is unreachable and the match is rejected.
