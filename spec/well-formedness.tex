...
entry for the variable to be $\Unbound$ or $\DeclTy{\tau}$, so a variable is declared once in its scope and
assigned at most once on any path, since a variable assigned on some path is unbound after the merge, as a blank final in Java~\cite[\S16]{gosling2025}, and a name bound to a
