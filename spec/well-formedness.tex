...
declares or assigns: a variable at its type if the statement \emph{definitely assigns} it, at $\DU{\tau}$ if it
declares it without assigning it, and at $\PU{\tau}$ if it assigns it on some paths only. The base cases are
that $\exReturn{e}$ definitely returns, that $x\exAnnot{\psi}$ declares $x$, and that $\exAssign{x}{e}$ and
$\exAssign{x\exAnnot{\psi}}{e}$ definitely assign it, at the declared type of $x$.}{Every well-typed statement either \emph{definitely returns}, or \emph{definitely assigns} a context
...
declaration with an assignment declares and assigns at once (\ruleName{declare-assign}). Each of the three requires the
entry for the variable to be $\Unbound$ or $\DU{\tau}$, so a variable is declared once in its scope and
assigned at most once on any path, since a variable assigned on some path is possibly unassigned after the merge, as a blank final in Java~\cite[\S16]{gosling2025}, and a name bound to a
class, module or predefined name cannot be declared or assigned. A closure captures only definitely assigned
...
prefers a locally \Replaced{bound}{assigned} name to one visible in an enclosing scope. \Added{The names of the region must be
undeclared where it occurs, and the names declared in a body must be distinct, as mypy requires.} The body is checked with the
...
imported name cannot be declared or assigned by the body. The names declared by the body must be distinct.}{overridden by the imported context, so the members of \pyinline{builtins} are in scope
unqualified.}
