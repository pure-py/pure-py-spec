...
\Replaced{A mutual region of function definitions (\ruleName{def}) resolves the annotations of each definition to a
...
assigned in the body start unbound, emulating}{A function definition binds its parameters at their annotated types, plus the names of its
mutual region, including the name of the function itself, each at the appropriate $\pyinline{Callable}$ type; variables declared or assigned in the body start
unbound (\ruleName{def}), emulating} Python's scoping of function bodies, which
...
since $\tySized$ has both list and tuple values\Added{, as does a type variable, which may stand for any type}.
...
overridden by the imported context and then, as in \Replaced{\ruleName{def-body}}{\ruleName{def}}, by an unbound entry for each name
