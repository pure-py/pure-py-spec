...
The matching judgement decides both reachability and exhaustiveness
(\figrefTwo{pat}{pat-seq}): a case is reachable when it matches some shape of the residual, and a
match is exhaustive when its last case leaves the residual empty. Because the residual is seeded
from the scrutinee's type, a case whose values the type already excludes is unreachable, as with
the uncovered sets of \citet{karachalias2015}. A single relation deciding both properties goes
back to the (``usefulness'') algorithm for ML~\cite{maranget2007}, and the residual threaded from
case to case corresponds to the term descriptions of the match compiler of \citet{sestoft1996}.
