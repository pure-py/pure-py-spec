...
\speconly{\Added{The rules for variables serve two aims: that a program has no mutable state, so a variable never
changes value, and that a well-typed program is also well typed under mypy. Within these limits the rules
permit as much as possible. A variable is introduced by a declaration, parameter, import, pattern or generator,
and receives its value at most once, with its declaration or by later assignment; an assignment never
introduces a variable and never changes one already assigned. A scope is the body of a module, function, lambda
or comprehension, as in Python, and the builtins enclose every module. A scope declares a name at most once and
may shadow a name of an enclosing scope, hiding it throughout the scope.}}
...
\Deleted{The rules are intended as a refinement of mypy's: a well-typed
...
builtin throughout, so a use of the name before its declaration is rejected, whereas mypy, following
Python's run-time lookup, resolves it to the builtin. As with any
