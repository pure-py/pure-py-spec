...
\Added{The rules for variables serve two aims: that a program has no mutable state, so a variable never changes
value, and that a well-typed program is also well typed under mypy. Within these limits the rules permit as
much as possible. For the first aim, a variable is introduced by a declaration, parameter, import, pattern or
generator, and receives its value at most once, with its declaration or by later assignment; an assignment never
introduces a variable and never changes one already assigned. Python rebinds a generator variable on each
iteration, so a lambda in a comprehension may not capture such a variable, since otherwise it would see a later
value. For the second aim, a language with lexical scoping may let a declaration reuse a name of its own scope,
introducing a new variable distinct from the earlier one, and whether to allow this is a matter of convenience
rather than of semantics. Since mypy rejects such a declaration as a redefinition, and compatibility with mypy
is a requirement, \PurePy rejects it too, whether the earlier name is that of a parameter, imported name,
pattern variable or other declaration. A name may still shadow a name of an enclosing scope or builtin, which
mypy permits.}
