...
A \pyinline{match} scrutinee synthesises a type $\sigma$, and the judgement
$\Sigma; \Gamma; \sigma \vdash \seq{p} \matches \seq{\Delta}, L$ matches each case's pattern in turn
against the values of $\sigma$ that the earlier cases have not matched, yielding each pattern's bindings
$\Delta_i$ and the unmatched values $L$ (\secref{well-formedness-patterns}). The body of each case is
