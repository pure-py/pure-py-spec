...
Each pattern must also be \emph{sequence-safe} at $\sigma$ (\secref{types-auxiliary}), because under the
evaluation rules a tuple pattern matches only a tuple and a list pattern matches only a list, whereas
Python matches both forms of pattern against both kinds of value (\figref{eval-pat}). A match on
...
The auxiliary function $\shapes(\Sigma, \tau, H)$ (\secref{types-auxiliary}) gives the shapes of $\tau$
