...
value. For the second aim, a declaration may not reuse a name of its own scope, whether of a parameter, imported
name, pattern variable or other declaration, since mypy rejects such reuse as a redefinition, whereas a name in a
nested scope may shadow a name of an enclosing scope or builtin, which mypy permits.}
...
the same scope. Each of these cases is rejected by mypy as a redefinition. The body of a lambda or nested definition is checked under the context in force where it is defined,
