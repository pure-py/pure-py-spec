...
declaration with an assignment declares and assigns at once (\ruleName{declare-assign}). An assignment requires the
entry for the variable to be $\DU{\tau}$, so a variable is assigned at most once on any path, since a variable
assigned on some path is possibly unassigned after the merge, following Java's rule for blank finals~\cite[\S16]{gosling2025}, and a
name bound to a class, module or predefined name cannot be assigned. That a variable is declared once in its scope
