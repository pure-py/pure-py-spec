...
unassigned or possibly unassigned after the statement.}{Every well-typed statement either \emph{definitely returns}, or \emph{definitely assigns} a context
...
\Replaced{A variable declaration $x\exAnnot{\psi}$ introduces $x$ at the type denoted by $\psi$, without assigning it
...
variable declaration with an assignment declares and assigns at once (\ruleName{declare-assign}). Since an assignment leaves the variable definitely assigned on that path, and possibly unassigned after a
...
bound to a class, module or predefined name cannot be assigned at all. The variable declaration rules do not consult the context for the variable being declared; instead \ruleName{def} and
