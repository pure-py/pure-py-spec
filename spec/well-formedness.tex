...
ordered set of shapes drawn from the scrutinee's type and covering every value not matched by any of the
earlier cases (\figrefTwo{pat}{pat-seq}). Matching the case removes from the residual the values matched
by $p$. The pattern $p$ must match at least one shape of the residual (\ruleName{pat-shapes}); otherwise
the case is unreachable and the match is rejected. A case for values outside the scrutinee's type is
unreachable in particular. The match is exhaustive if the last case leaves the residual empty, and may
otherwise fall through (\ruleName{match}, \ruleName{match-partial}). Reading both properties off one
relation goes back to the ``usefulness'' algorithm for ML~\cite{maranget2007}. The residual plays the
