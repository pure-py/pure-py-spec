...
separate test; without a type system to bound the values a match may receive, it degenerates to
whether the last case is a catch-all, a variable or wildcard.

\specinput{spec/fig/subsumption}

A pattern list is well-formed (\figref{well-formed-pat}) when each pattern is linear (no pattern variable occurs more than once) and no earlier pattern subsumes a later one. The \pyinline{match} well-formedness rules use this judgement to enforce reachability and linearity of case patterns.
