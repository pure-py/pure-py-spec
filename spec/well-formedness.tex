...
class of $H$, whose fields, those of $C$, have shapes $\seq{k}$.
...
Both reachability and exhaustiveness are determined by the residual left by matching.
...
is first split by the auxiliary judgement $\Sigma; \Gamma \vdash k, p \splits K, L$ (\figref{split}):
the shapes $K$ have the head tested by $p$, with the shapes of the component types drawn from $\shapes$,
and the residual $L$ is the shape $k$ with that head excluded as well. \ruleName{pat-split} matches the
pattern against $K$ and adds $L$ to the residual. Only the forms
...
The evaluation rules apply a tuple or list pattern only to a value
