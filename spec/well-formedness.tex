...
The statement judgement $\Sigma; \Gamma; \zeta \vdash s : R$ says that $s$ has static outcome $R$ under $\Gamma$
and optional return type $\zeta$ (\figref{typing-stmt}); every rule passes $\zeta$ down unchanged.
A sequence $s\;s'$ (\ruleName{seq}) types $s$, types $s'$ under $\Gamma$ overridden by the context that $s$
