...
$\shapesOf(\Sigma, \sigma, \varnothing)$, which denotes every value of $\sigma$. The residual $L_i$ holds
...
To check a \pyinline{match}, each case is matched in turn against the \emph{residual}: those parts of the
scrutinee's type which no earlier case has matched. A case that matches none of the
residual is unreachable, and a match whose last case leaves an empty residual is exhaustive, so a
single judgement decides both properties (\figrefTwo{pat}{pat-seq}). Because the residual starts
from the type, a case whose values the type excludes is unreachable. A single relation deciding both
properties goes back to the ``usefulness'' algorithm for ML~\cite{maranget2007}. The residual
plays the role of the term descriptions of \citet{sestoft1996}, and the uncovered sets of
\citet{karachalias2015} likewise let the type exclude cases.

The residual is represented as a finite set of shapes. A \emph{shape} $k$
...
\input{spec/fig/value-descriptors}

\input{spec/fig/value-descriptor-typing}

\input{spec/fig/descriptor-entries}

\input{spec/fig/pat-descriptors}

\input{spec/fig/pat-descriptors-heads}
