...
value. The second aim settles one question, whether a declaration may reuse a name of its own scope.
Under lexical scoping such reuse introduces a new variable, distinct from the earlier one, so a language is
free to allow it or not as a matter of convenience. Because mypy rejects it as a redefinition, \PurePy rejects
it too, whether the earlier name is that of a parameter, imported name, pattern variable or other declaration.
A name may still shadow a name of an enclosing scope or builtin, which mypy permits.}
