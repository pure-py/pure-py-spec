...
judgement $\Sigma; \Gamma; \zeta; \sigma \vdash L, \seq{h} : S, L'$ (\figref{typing-cases}), which takes
the cases in turn: each case's pattern is matched against the \emph{residual} $L$, a static
over-approximation of the values of $\sigma$ which are not matched by the earlier cases
(\secref{well-formedness-patterns}), its body is checked under the pattern's bindings, and the outcomes
are merged into $S$. Each pattern must agree with $\sigma$ (\figref{agrees}). The match rules start the
residual from $\sigma$; \ruleName{match} applies where the final residual is empty; \ruleName{match-partial} applies otherwise, merging in
