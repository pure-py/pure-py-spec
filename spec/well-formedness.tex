...
type expression denotes a type under a context\Added{ and class table} (\figref{resolve-type}). Every definition carries an
annotation for its parameters and return type, so the type of a definition is given rather than
inferred from its body. Within a type expression, \Replaced{a class name applied to as many type arguments as the class has parameters resolves to an instantiated class, an alias name applied to type arguments resolves to the instantiation of its alias, type parameters in scope denote the type variables of those names}{class names resolve to classes, type parameters in scope denote the type variables of those names}, and all other names must be predefined names (\figref{resolve-type}). The function $\baseType(\tau)$ discards the precision of a
literal type, taking $\tyLiteral{42}$ to \pyinline{int} and otherwise acting as the identity
...
Subtyping (\figref{subtyping}) has every type below itself, a literal type below its base type, and an instantiated class $C[\seq{\tau}]$ below every type that its base class is below, once $\seq{\tau}$ is substituted into the arguments of the base (\Replaced{\ruleName{subty-class-extend}}{\ruleName{subty-class}})\Replaced{, and below $C[\seq{\sigma}]$ when $\seq{\tau}$ and $\seq{\sigma}$ are equivalent (\ruleName{subty-class}), so that type arguments are invariant, as for lists and dictionaries}{, so that a class applied to distinct type arguments is related to itself by reflexivity alone and type arguments are invariant}; a type variable is below itself and \pyinline{object} alone, with \pyinline{object} above every type and \pyinline{Never} below every type. A union is above
each of its members and below every type that is above all of them. A tuple type is covariant, below another of the
same length when each of its components is below the corresponding component. Two types are \emph{equivalent}, $\sigma \tyEquiv \tau$, when each is a subtype of the other; the notations $\subty$ and $\tyEquiv$ follow \citet{pierce2002}. Lists and
dictionaries are related only when their components are equivalent, since both are invariant in mypy. A callable type is contravariant in its parameters
...
So joining a type with itself gives that type, joining a class with \Replaced{an instantiation of one of its ancestors above it gives that instantiation}{one of its ancestors gives the ancestor}, and
...
The fields of a class $C$ form a field scheme $\fields(\Sigma, C)$ over its type parameters, with one binding $\bind{x}{\tau}$ per
field: the fields of its base class first, recursively and instantiated at the type arguments of the base, then the fields declared by $C$ itself, each at its
declared type; the fields of an instantiated class $C[\seq{\tau}]$ are the instantiation $\tyInst{\fields(\Sigma, C)}{\seq{\tau}}$. The order matters, because a constructor call pairs positional arguments with fields by
...
synthesise \Replaced{an instantiated class $C[\seq{\tau}]$, and $\exAttr{e}{x}$ then has the type bound to $x$ in $\tyInst{\fields(\Sigma, C)}{\seq{\tau}}$}{a class $C$, and $\exAttr{e}{x}$ then has the type bound to $x$ in $\fields(\Sigma, C)$}
...
& \Sigma \vdash C[\seq{\sigma}] \eqType && \qquad\quad \text{if } \Sigma \vdash \tau \eqType \text{ for each } \bind{x}{\tau} \in \tyInst{\fields(\Sigma, C)}{\seq{\sigma}}
\end{defalign}
...
the class name to $\Type{C}$, resolves the field annotations with its type parameters in scope, and requires the fields it declares to be distinct and, when it extends
another class, \Replaced{the base to resolve to an instantiated class}{the base name to be bound to a class, applied to as many type arguments as that class has parameters}, and the additional fields not to repeat an inherited field
