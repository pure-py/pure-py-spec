...
inferred from its body. \Replaced{Within a type expression, class names resolve to classes, type parameters in scope denote the type variables of those names, and all other names must be predefined names (\figref{resolve-type}).}{A class name resolves to a
class (\figref{resolve-name}) and every other name must be a predefined name
(\figref{predefined-signatures}).} The function $\baseType(\tau)$ discards the precision of a
literal type, taking $\tyLiteral{42}$ to \pyinline{int} and \Replaced{otherwise acting as the identity}{leaving every other type alone}
...
same length when each of its components is below the corresponding component. Two types are \emph{equivalent}, $\sigma \tyEquiv \tau$, when each is a subtype of the other; \Replaced{the notations $\subty$ and $\tyEquiv$}{both notations} follow \citet{pierce2002}. Lists and
dictionaries are related only when their components are equivalent, since both are invariant in mypy\Deleted{
...
\Replaced{The fields of a class $C$ form a field scheme $\fields(\Sigma, C)$ over its type parameters, with one binding $\bind{x}{\tau}$ per
...
declared type; the fields of an instantiated class $C[\seq{\tau}]$ are the instantiation $\tyInst{\fields(\Sigma, C)}{\seq{\tau}}$.}{The fields of a class $C$ form a sequence $\fields(\Sigma, C)$ of bindings $\bind{x}{\tau}$, one per
...
& \Sigma \vdash \replaced{C[\seq{\sigma}]}{C} \eqType && \qquad\quad \text{if } \Sigma \vdash \tau \eqType \text{ for each } \bind{x}{\tau} \in \replaced{\tyInst{\fields(\Sigma, C)}{\seq{\sigma}}}{\fields(\Sigma, C)}
\end{defalign}
