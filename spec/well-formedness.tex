...
assigns $x$ at once (\ruleName{declare-assign}). An assignment leaves $x$ definitely assigned on that path, and
possibly unassigned after a merge with a path that does not assign it, so a variable is assigned at most once on
any path; a class name, module name or predefined name cannot be assigned at all. The variable declaration rules
do not consult the context for the variable being declared; instead \ruleName{def} and \ruleName{module} require
the scope to be well-scoped (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:well-scoped}}): the
names declared in the scope, including the parameters of a function or the imported names of a module, must be
distinct from one another and from any pattern variables in the scope. The body of a lambda or nested definition
is checked at its point of definition, so a variable of the enclosing scope referenced by the body must be
definitely assigned at that point, whereas Python resolves the reference only when the body runs.}{An annotated assignment checks the expression against the type of the annotation,
