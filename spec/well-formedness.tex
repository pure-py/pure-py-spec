...
bounds $(\tau_1, \tau_2)$. The unary set $\overloadsOf(\Sigma, \ominus, \sigma_1)$ is defined in the same way from
conclusions $\Sigma \vdash \ominus(\sigma_1) : (\tau_1, \tau)$. In both cases $\min$ denotes the least element
...
The residual is represented as a finite ordered set of shapes. A \emph{shape} $k$
...
The
matching judgement $\Sigma; \Gamma \vdash L, p \matches K, L' \binding \Delta$ divides an ordered set of shapes into the
