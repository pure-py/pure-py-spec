...
The case rules (\figref{typing-cases}) also require each pattern to be \emph{sequence-safe} at the
type of the scrutinee (\secref{types-auxiliary}), because under the evaluation rules a tuple pattern
matches only a tuple and a list pattern matches only a list, whereas Python matches both forms of
pattern against both kinds of value (\figref{eval-pat}). A match on
