...
\PurePy program should be accepted by mypy, and \PurePy may reject programs accepted by mypy but not the
...
class (\figref{resolve-name}) and every other name must resolve to a predefined name
...
under this order, if one exists.
\end{definition}
\end{DeletedBlock}

\noindent The \ruleName{binop} and \ruleName{unop} rules (\figref{typing-synth}) take the result type
of \Replaced{the most specific overload at the operand types, $\resolveOp$ (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:resolve-op}})}{$\min(\overloadsOf(\Sigma, \odot, \sigma_1, \sigma_2))$}, so \pyinline{1 + 2} is an
...
overloads give \pyinline{Any} whenever the exponent is not a literal, whereas the \Replaced{most specific}{least} overload resolves to
\pyinline{float}.

\begin{lemma}[\replaced{Most specific}{Least} overload]
\label{lem:most-specific-overload}
\Replaced{If neither $\sigma_1$ nor $\sigma_2$ is a subtype of \pyinline{Never} and some overload of $\odot$ applies at
$(\sigma_1, \sigma_2)$, then $\resolveOp(\Sigma, \odot, \sigma_1, \sigma_2)$ is defined; likewise
$\resolveOp(\Sigma, \ominus, \sigma)$.}{If neither $\sigma_1$ nor $\sigma_2$ is a subtype of \pyinline{Never} and
...
\Replaced{A well-typed statement has a \emph{static outcome} $R$: either $\Returns$, when the statement \emph{definitely
returns}, or $\Assigns{\Delta}$ for a context $\Delta$, possibly empty, recording the variables the statement
declares or assigns (Definition~\ref{def:context}): a variable at its type if the statement \emph{definitely
assigns} it, at $\DU{\tau}$, definitely unassigned, if it declares it without assigning it, and at $\PU{\tau}$,
possibly unassigned, if it assigns it on some paths only.}{Every well-typed statement either \emph{definitely returns}, or \emph{definitely assigns} a context
$\Delta$, which may be empty. The base cases are that \pyinline{return e} definitely returns, and \pyinline{x = e} definitely assigns
$\set{\bind{\pyinline{x}}{\sigma}}$ for the type $\sigma$ of \pyinline{e}. \emph{Static outcomes} $R$ classify the two completions: $R$ is either $\Returns$ (definitely returns) or $\Assigns{\Delta}$ (definitely assigns $\Delta$).}
...
A sequence $s\;s'$ (\ruleName{seq}) types $s$, \Added{then} types $s'$ under $\Gamma$ overridden by the context assigned
by $s$\Deleted{, and requires that no variable captured by $s$ is assigned in $s'$}. A conditional combines its
...
(\ruleName{top-stmt}), so a \pyinline{return} has no derivation at the top level. \Deleted{A top-level
...
\pyinline{return} \Replaced{requires \pyinline{None} to be a subtype of $\tau$}{requires $\tau$ to admit \pyinline{None}}.

\Added{The rules for variables serve two aims: that a program has no mutable state, so a variable never
changes value, and that a well-typed program is also well typed under mypy. Within these limits the rules
permit as much as possible. For the first aim, a variable is introduced by a declaration, parameter, import,
pattern or generator, and receives its value at most once, with its declaration or by later assignment; an
assignment never introduces a variable and never changes one already assigned. Python rebinds a generator
variable on each iteration, so a lambda in a comprehension may not capture such a variable, since otherwise it
would see a later value. The second aim concerns scope. A notion of scope usually determines the parts of a
program within which names must be unique, so that a scope declares a name at most once. Two choices then
remain to a language: how a program decomposes into scopes, and whether a scope may shadow a name of an
enclosing scope. A \PurePy program is a Python program, so a scope is the body of a module, function, lambda
or comprehension, and the builtins enclose every module. Python itself does not distinguish definition from
assignment: a name is defined by assigning it, and assigning it again reassigns the existing variable. mypy,
by contrast, treats an annotation as a definition, and rejects a declaration whose name is already that of a
parameter, imported name, pattern variable or declaration of the same scope. On the second choice mypy permits
shadowing, and so does \PurePy. A function body, for example, is a single scope, so it cannot declare a name
twice, whether on one path or in different branches, nor redeclare one of its parameters; since shadowing is
permitted, it may declare a name of an enclosing body or a builtin. Since a name denotes one variable throughout
a scope, a declaration hides the enclosing variable everywhere in the scope, so a use before the declaration
finds the name unbound rather than referring to the enclosing variable. mypy agrees, with one exception: at
module level a use before the declaration of a builtin name refers to the builtin, as it does when Python runs
the module. \PurePy makes no exception, treating the builtins as an enclosing scope like any other, and so
rejects a program that mypy accepts. A pattern introduces its variables rather than assigning them, so a pattern
cannot bind a name declared in its scope either, although several cases of one match may bind the same
variable.}

\Replaced{A declaration $x\exAnnot{\psi}$ introduces $x$ at the type denoted by $\psi$, without assigning it
(\ruleName{declare}); an assignment requires the variable to be
declared and not yet assigned, and checks the expression against the declared type (\ruleName{assign}); a
declaration with an assignment declares and assigns at once (\ruleName{declare-assign}). Since an assignment leaves the variable definitely assigned on that path, and possibly unassigned after a
merge with a path that does not assign it, a variable is assigned at most once on any path, following Java's
rule for blank finals~\cite[\S16]{gosling2025}; a name
bound to a class, module or predefined name cannot be assigned at all. The declaration rules do not consult the context for the variable being declared; instead \ruleName{def} and
\ruleName{module} require the scope to be well-scoped (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:well-scoped}}): its declarations, including the parameters of a function or the imported
names of a module, must be distinct from one another and from any pattern variables in the scope. A name therefore cannot be declared twice in a scope,
whether on one path or in different branches, nor can a declaration share its name with a pattern variable of
the same scope. The body of a lambda or nested definition is checked under the context in force where it is defined,
so a variable of the enclosing scope referenced by the body must be definitely assigned at that point. Since
no variable is assigned twice, the value captured by a closure never changes\paperonly{ (\secref{no-mutable-state})}.}{An annotated assignment checks the expression against the type of the annotation,
and an unannotated assignment takes the type synthesised by the expression (\ruleName{assign},
\ruleName{assign-unannot}).}
...
the class name to $\Type{C}$, requiring the fields it declares to be distinct and, when it extends
...
A function definition binds its parameters at their annotated types, plus the names of its
mutual region, including the name of the function itself, each at the appropriate $\pyinline{Callable}$ type; variables \Added{declared or} assigned in the body start
\replaced{unbound}{at $\False$} (\ruleName{def}), emulating Python's scoping of function bodies, which
prefers a locally \Replaced{bound}{assigned} name to one visible in an enclosing scope. \Added{As with any scope, the declarations of the function, including its parameters, must be distinct from one
another and from any pattern variables in its body. The function names of the mutual region are not among these; they are
declarations of the enclosing scope, where the same condition applies to them.} The body is checked with the
...
Python matches both forms of pattern against both kinds of value\speconly{ (\figref{eval-pat})}. \Replaced{Neither a tuple pattern nor a list pattern is therefore sequence-safe at
$\tyList{\tyInt} \mathbin{|} \tyTuple{\tyInt, \tyInt}$, and a tuple pattern is not sequence-safe at
$\tySized \mathbin{|} \tyTuple{\tyInt, \tyInt}$.}{A match on
$\tyList{\tyInt} \mathbin{|} \tyTuple{\tyInt, \tyInt}$ therefore admits no tuple or list pattern, and a
match on $\tySized \mathbin{|} \tyTuple{\tyInt, \tyInt}$ admits no tuple pattern.}
...
\Replaced{overridden by the imported context, with each name assigned in the body starting unbound, following
\ruleName{def}, so the members of \pyinline{builtins} are in scope unqualified and a name bound by the body shadows a
builtin throughout. As with any
scope, the declarations of the module, including its imported names, must be distinct from one another and
from any pattern variables in its body.}{overridden by the imported context, so the members of \pyinline{builtins} are in scope
unqualified.}
