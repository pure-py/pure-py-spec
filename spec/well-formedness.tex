...
entry for the variable to be $\Decl$ or $\DeclTy{\tau}$, so a variable is declared once in its scope and
assigned at most once on any path, as a blank final in Java~\cite[\S16]{gosling2025}, and a name bound to a
...
at $\Decl$ (\ruleName{def}), emulating Python's scoping of function bodies, which
...
\Replaced{overridden by $\Decl$ for each name the body binds and then by the imported context, so the members of
