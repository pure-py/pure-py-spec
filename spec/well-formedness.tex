...
\Replaced{The fields of a class $C$ form a scheme $\fields(\Sigma, C)$ over its type parameters, whose body is a sequence of bindings $\bind{x}{\tau}$, one per
...
declared type; an instantiated class $C[\seq{\tau}]$ has the fields $\tyInst{\fields(\Sigma, C)}{\seq{\tau}}$.}{The fields of a class $C$ form a sequence $\fields(\Sigma, C)$ of bindings $\bind{x}{\tau}$, one per
