...
\pyinline{x = e} definitely assigns $\set{\bind{\pyinline{x}}{\tau}}$ for the declared type $\tau$ of
\pyinline{x}. \emph{Static outcomes} $R$ classify the two completions: $R$ is either $\Returns$ (definitely
...
\pyinline{return} requires $\tau$ to admit \pyinline{None}. A bare annotation declares a variable at the type
...
variables, which are never assigned again, so a closure sees a fixed value\paperonly{ (\secref{no-mutable-state})}. The conditions of \pyinline{if} and \pyinline{assert}
...
prefers a locally bound name to one visible in an enclosing scope. The names of the region must be
...
overridden by $\Unbound$ for each name the body binds and then by the imported context, so the members of
\pyinline{builtins} are in scope unqualified, a name bound by the body shadows a builtin throughout, and an
imported name cannot be declared or assigned by the body. The names declared by the body must be distinct.
