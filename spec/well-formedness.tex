...
of \Replaced{the least overload at the operand types, $\resolveOp$ (\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:resolve-op}})}{$\min(\overloadsOf(\Sigma, \odot, \sigma_1, \sigma_2))$}, so \pyinline{1 + 2} is an
...
overloads give \pyinline{Any} whenever the exponent is not a literal, whereas the least overload resolves to
\pyinline{float}.
