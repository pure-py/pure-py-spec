...
A \pyinline{match} scrutinee synthesises a type $\sigma$, and the cases are checked using the auxiliary
judgement $\Sigma; \Gamma; \sigma \vdash \seq{p} \matches \seq{\Delta}, L$
(\secref{well-formedness-patterns}), which matches each case's pattern in turn against the values of
$\sigma$ that the earlier cases have not matched, yielding each pattern's bindings $\Delta_i$ and the
unmatched values $L$. The body of each case is
checked under its pattern's bindings, and each pattern must agree with $\sigma$ (\figref{agrees}). The
\ruleName{match} rule applies where $L$ is empty; \ruleName{match-partial} applies otherwise, merging in
$\Assigns{\varnothing}$ for the values that fall through. A literal-typed scrutinee is exhausted by covering its literals, so \pyinline{x = 42} admits a
