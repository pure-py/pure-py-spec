...
(\ruleName{class}, \ruleName{class-extend}). It also requires the predefined name \pyinline{dataclass} (\figref{predefined-types}) to be in
scope; every \PurePy class must declare itself a dataclass. Each rule that takes an annotation resolves it to a type
(\figref{resolve-type}).

A function definition binds its parameters at their annotated types and its own name, as well as the names of the
mutual region it belongs to, each at its $\pyinline{Callable}$ type; variables assigned in the body start
at $\statusNotDefAssigned$ (\ruleName{def}), emulating Python's scoping of function bodies, which
