...
denotes a set of values of its type $\shapeTy(k)$ (\secref{types-auxiliary}). The form $\shapeAny{\tau}{H}$ is a
value of type $\tau$ whose outermost head is not among those in $H$. A head itself denotes values: a
...
$\shapesOf(\Sigma, \tau, H)$: one shape per member of a union, and none at all where $H$ exhausts $\tau$. No shape carries a
union or dictionary type in the form $\shapeAny{\tau}{H}$, since $\shapesOf$ is the only source of
...
\shapesOf(\Sigma, \tau, H)$. Moreover, if $\Sigma \vdash k : \tau$ for each $k \in L$ and $\Sigma; \Gamma \vdash L, p
...
neither $\beta$ nor $H$ splits the shape at that key (\ruleName{split-key}), adding to the residual the
