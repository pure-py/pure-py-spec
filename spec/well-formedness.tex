...
are merged into $S$. Each pattern must be sequence-safe at $\sigma$ (\figref{seq-safe}). The match rules start the
...
earlier cases (\figref{pat}). Matching the case removes from the residual the values matched
...
therefore be \emph{sequence-safe} at the scrutinee's type (\figref{seq-safe}), so a match on
