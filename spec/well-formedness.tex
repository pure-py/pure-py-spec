...
declaration with an assignment declares and assigns at once (\ruleName{declare-assign}). An assignment requires the entry for the variable to be $\DU{\tau}$. A variable is therefore assigned at most
once on any path, because an assignment on some path leaves it possibly unassigned after the merge, following
Java's rule for blank finals~\cite[\S16]{gosling2025}. A name bound to a class, module or predefined name cannot
be assigned at all. That a variable is declared once in its scope
is a condition on the scope (\ruleName{def}, \ruleName{module}). The body of a lambda or nested definition is checked under the context in force where it is defined,
