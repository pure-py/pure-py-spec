...
scrutinee's type (\secref{types-auxiliary}), because the evaluation rules apply a tuple or list pattern
only to a value of its own kind, although Python would match either (\figref{eval-pat}). A match on
$\tyList{\tyInt} \mathbin{|} \tyTuple{\tyInt, \tyInt}$ therefore admits no tuple or list pattern, and a
match on $\tySized \mathbin{|} \tyTuple{\tyInt, \tyInt}$ no tuple pattern.
