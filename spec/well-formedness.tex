...
entry for the variable to be $\False$ or $\FalseTy{\tau}$, so a variable is declared once in its scope and
...
\Replaced{overridden by $\False$ for each name the body binds and then by the imported context, so the members of
