...
class (\figref{resolve-name}) and every other name must \Replaced{be}{resolve to} a predefined name
...
(\ruleName{declare}); an assignment $\exAssign{x}{e}$ requires $x$ to be declared and not yet assigned, and
checks $e$ against the declared type (\ruleName{assign}); the form $\exAssign{x\exAnnot{\psi}}{e}$ declares and
assigns $x$ at once (\ruleName{declare-assign}). Since an assignment leaves the variable definitely assigned on that path, and possibly unassigned after a
merge with a path that does not assign it, a variable is assigned at most once on any path, following Java's
rule for blank finals~\cite[\S16]{gosling2025}; a class name,
module name or predefined name cannot be assigned at all. The variable declaration rules do not consult the context for the variable being declared; instead \ruleName{def} and
