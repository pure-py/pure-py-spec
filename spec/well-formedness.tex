...
values of type $\tau$ whose outermost head is not among those in $H$; the other forms are constructors,
tuples, lists and dictionaries, each with shapes in place of sub-patterns. A list shape also records its
...
class of $H$, whose fields, those of $C$, have the shapes $\seq{k}$.
...
The shapes of $\tau$ that remain once the heads in $H$ are excluded are
