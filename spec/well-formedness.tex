...
(\paperonly{\figref{auxiliary-functions}}\speconly{\S\ref{aux:scope}}).}{An annotated assignment checks the expression against the type of the annotation,
...
mutual region, including the name of the function itself, each at the appropriate $\pyinline{Callable}$ type; variables \Replaced{declared in the body or bound by a pattern in it}{assigned in the body} start
\replaced{unbound}{at $\False$} (\ruleName{def}\Added{, $\scopeS(\set{\seq{x}}, s)$}), emulating Python's scoping of function bodies, which
prefers a locally \Replaced{bound}{assigned} name to one visible in an enclosing scope. \Added{A variable of the
...
Python matches both forms of pattern against both kinds of value\speconly{ (\figref{eval-pat})}. \Replaced{A tuple pattern is therefore sequence-safe only at a type with no list values, and a list pattern only
...
since $\tySized$ has both list and tuple values.}{A match on
...
\Replaced{overridden by the imported context and then, as in \ruleName{def}, by an unbound entry for each name
declared or assigned in the body ($\scopeS$). The members of \pyinline{builtins} are thus in scope unqualified, and a name bound by the body
...
body.}{overridden by the imported context, so the members of \pyinline{builtins} are in scope
unqualified.}
