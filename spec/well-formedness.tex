...
set: $\shapeAny{\patConstr{C}{\seq{k}}}{H}$ denotes the instances of $C$, or of a subclass not below a
...
shape, giving the matched shapes $K$, residual $L$ and bindings $\Delta$; the form on sequences
matches the sub-patterns of a tuple, list, dictionary or constructor pattern against the component
shapes position by position; and the form on an ordered set of shapes $L$, used by the match rules,
matches each shape of $L$ in turn. Some shapes of $L$ may admit no derivation for the pattern, so
\ruleName{pat-shapes} takes the shapes of $L$ that do admit a derivation, of which there must be at least
one, and passes the remaining shapes into the residual untouched. A variable pattern binds at $\shapeTy(k)$ for the shape $k$ it matches, and a
