...
is a condition on the scope (\ruleName{def}, \ruleName{module}). A closure captures only definitely assigned
variables, which are never assigned again, so a closure sees a fixed value\paperonly{ (\secref{no-mutable-state})}.}{An annotated assignment checks the expression against the type of the annotation,
