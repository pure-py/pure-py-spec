...
To check a \pyinline{match}, each case is matched in turn against the residual: the values of the
scrutinee's type which have not been matched by any earlier case. A case that matches none of the
...
from the type, a case whose values the type excludes is unreachable. The (``usefulness'') algorithm
for ML also decides both properties with a single relation~\cite{maranget2007}; the residual
corresponds to the term descriptions a match compiler threads through the
cases~\cite{sestoft1996}; and starting from the type follows the uncovered sets of
\citet{karachalias2015}.
