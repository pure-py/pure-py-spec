...
declared and not yet assigned, and checks the expression against the declared type (\ruleName{assign});
the form $\exAssign{x\exAnnot{\psi}}{e}$ declares and assigns at once (\ruleName{declare-assign}). Since an assignment leaves the variable definitely assigned on that path, and possibly unassigned after a
