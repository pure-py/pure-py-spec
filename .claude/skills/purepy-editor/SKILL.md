...
- Table cells, figure captions and rule side conditions are telegraphic: no articles ("Rebinds variable", not "Rebinds a variable"; "neither list type nor union"). A gloss in apposition after a comma or colon within a cell is ordinary prose and keeps its articles ("head, a literal, a class or a list of length n"; "context, a finite map from variables to entries").
