...
The \emph{dependency graph} $G_{\mathcal{M}}$ of a module map is the directed graph whose edges $(M, M')$ are
those for which either $M \in \dom(\mathcal{M})$ and $M' \in \imports(\mathcal{M}(M))$, or $M' \in
\parents(M)$. Program well-formedness (\figref{modules}) requires every module in $\mathcal{M}$ to be
well-formed in its initial context, and $G_{\mathcal{M}}$ to be acyclic.
