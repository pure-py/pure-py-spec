...
$\nameOf(\pymath{\_\_main\_\_}) = \pymath{"\_\_main\_\_"}$. A module body is a sequence of imports
...
The metafunction $\assignsS$ extends from individual statements to blocks and module bodies by union, and $\imports$ from individual imports to module bodies by union over the import prefix. The metafunction $\parents$ collects the proper prefixes of a
...
to be acyclic. The \emph{import closure} of a module $q$ is the set of modules reachable from $q$ in
...
Suppose $\mathcal{M}(q) = \vec{\iota}\;\vec{s}$ with $\vdash \vec{\iota} : \Gamma$ and $\Gamma \vdash \bodyOf(q) : \Delta$. Define the \emph{exports} of $q$ as $\Delta$, and the \emph{context} of $q$ as $\Gamma_q = \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \Delta$.
\end{definition}
