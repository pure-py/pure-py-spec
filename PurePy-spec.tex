...
\item \emph{class entry} $\clsEntry{q}{\vec{x}}{c'}$, of the defining module $q$, a sequence of distinct field names $\vec{x}$, and a base class $c'$ (or $\bot$ for no base).
\end{itemize}
\end{definition}
...
if $q \in \dom(\mathcal{M})$ then $\parents(q) \subseteq \dom(\mathcal{M})$. The pair $\langle E, \mathcal{M} \rangle$
...
\begin{definition}[Variable context of a well-formed module]
For a well-formed module $q$, i.e.\ where $\Gamma_q^0 \vdash \mathcal{M}(q) : \Delta$ for some $\Delta$, the \emph{variable context} of $q$ is $\Gamma_q = \Gamma_q^0 \runion \Delta$.
\end{definition}
...
A module is evaluated only after it has been judged well-formed (\figref{well-formed-module}). Well-formedness of a module $q$ holds in its variable context $\Gamma_q$, whose entries are definite-assignment statuses, module references, and class entries. Evaluation instead proceeds in a runtime environment $\rho$ binding names to values. The two are related by the projection $\lfloor \Gamma \rfloor$, which removes the class entries of $\Gamma$ and keeps its statuses and module references: a module well-formed in $\Gamma_q$ is evaluated in an environment with domain $\dom(\lfloor \Gamma_q \rfloor)$. Classes are therefore compile-time only. They are consulted when checking construction and pattern matching, but are never bound at runtime (\ruleName{eval-class} and \ruleName{eval-class-extend} extend the environment by nothing), since classes are not first-class values.
...
$\rho \vdash p, v \matches \rho'$ (\figref{match}) is a deterministic partial function; we write $\rho \vdash p, v \notmatches$
(with no right-hand side) for $\nexists \rho'.\, \rho \vdash p, v \matches \rho'$, i.e.\ matching is undefined.
