...
\item a \emph{module reference} $\modEntry{q}$, for a fully-qualified module name $q$;
...
%
\begin{definition}[Override]
{\small
\begin{align*}
\bot \runion \theta &= \theta \runion \bot = \theta \\
\theta \runion \theta' &= \theta' \quad (\text{when } \theta' \neq \bot)
\end{align*}}
\end{definition}
...
$\nameOf(\pymath{\_\_main\_\_}) = \pymath{"\_\_main\_\_"}$. \Added{We identify a module with its fully-qualified name, writing ``module $q$'' for the module named $q$.} \Added{A module body is a sequence of imports
...
well-formedness and evaluation rules check and evaluate \Added{the statements of} each body $\mathcal{M}(q)$ with the
assignment $\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}$ prepended \Added{(metafunction $\bodyOf$,
\figref{eval-expr})}, modelling the $\pymath{\_\_name\_\_}$ binding the runtime supplies
automatically\Deleted{\ rather than one written in the source}. $\mathcal{M}$ additionally always includes the predefined modules of
...
\Replaced{The metafunction $\assignsS$ extends by union from individual statements to blocks, and $\imports$ from individual imports to import sequences; on a module body $\vec{\iota}\;\vec{s}$, they act on $\vec{s}$ and $\vec{\iota}$ respectively.}{The metafunction $\assignsS$ extends from individual statements to blocks and module bodies by union, and $\imports$ to module bodies.} The metafunction $\parents$ collects the proper prefixes of a
fully-qualified name.

\input{fig/modules}

The \emph{dependency graph} $G_{\mathcal{M}}$ of a module map has an edge $(q, q')$ iff $q' \in
...
to be acyclic. \Added{The \emph{dependencies} $\deps(q)$ of a module $q$ (\figref{modules}) are the modules
...
\begin{definition}[Members of a well-formed module]
\Replaced{Suppose $\mathcal{M}(q) = \vec{\iota}\;\vec{s}$ with $\vdash \vec{\iota} : \Gamma$ and $\Gamma \vdash \bodyOf(q) : \Delta$.}{Suppose $\varnothing \vdash \bodyOf(q) : \Delta$.} Define the \emph{exports} of $q$ as $\Delta$, and the \Replaced{\emph{members} of $q$ as $\members(q)$}{\emph{context} of $q$ as $\Gamma_q$} $= \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \Delta$.
\end{definition}

\noindent The exports $\Delta$ comprise the classes and variables a module defines, together with its intrinsic \pyinline{__name__}, never the names it imports: following PEP 484\footnote{\url{https://peps.python.org/pep-0484/}}, an imported name is a private dependency, not re-exported as part of the interface. \Replaced{The members $\members(q)$ also contain}{The context $\Gamma_q$ also contains} an entry for each submodule $q.x$\Added{; the two parts are disjoint, because the \ruleName{module} rule prohibits a module from binding a name that is also the name of one of its submodules (\S\ref{sec:submodule-clash})}.

\Added{Rule \ruleName{qualified-submodule} requires the submodule to be among the dependencies $\deps(q_0)$ of the referring module $q_0$, ensuring it is loaded wherever the reference can be evaluated (\S\ref{sec:attribute-error}).}
...
\item \Replaced{Local or interleaved imports: an \pyinline{import} or \pyinline{from} statement inside a function or other block, or after a non-import statement. \PurePy permits imports only at the start of a module; Python allows them anywhere.}{Local imports: an \pyinline{import} or \pyinline{from} statement inside a function or other block. \PurePy permits imports only at module level; Python allows them anywhere.}
...
\item \Added{A module binding a name that is also the name of one of its submodules (\S\ref{sec:submodule-clash}).}
\item \Added{Referring to a submodule the referring module does not itself import: \pyinline{import a} followed by \pyinline{a.b.f()} is rejected unless \pyinline{a.b} is among the module's own dependencies. Python permits the reference whenever some earlier import, anywhere in the program, loaded \pyinline{a.b}.}
\end{itemize}
...
\ruleName{module} rule prohibits a module from binding a name that is also the name of one of its submodules.
\end{AddedBlock}

\begin{AddedBlock}
\subsection{Python errors prevented by well-formedness}
\label{sec:ruled-out}

A well-formed \PurePy program avoids several classes of Python runtime error by construction; this section
collects them, with the rules responsible. (The previous section is the complementary list: programs Python
runs that \PurePy rejects.) Errors from ill-typed operations, such as wrong operand types, calling a
non-function, or attribute access on an object lacking the attribute, are outside the scope of
well-formedness.

\subsubsection{\pyinline{NameError} and \pyinline{UnboundLocalError}}

Referencing a variable that is not definitely assigned is ill-formed (rule \ruleName{var}): conditionals and
\pyinline{match} statements merge their branches' contexts, so a variable assigned in only some branches
cannot be referenced afterwards (\S\ref{sec:local-assignment}). Python raises \pyinline{NameError} at module
level and \pyinline{UnboundLocalError} inside a function when the unassigned path is taken. Definite
assignment gives each reference a unique lexical referent: a variable is introduced by an assignment
(\S\ref{sec:assignment-as-definition}), possibly jointly by the branches of a conditional, and a name may be
referenced only where one such introduction is in scope on every path. This is stricter than avoiding the
errors above: the left-hand program of \S\ref{sec:contexts} assigns \pyinline{x} on every run, but is
rejected because the reference would denote the branch-local variable on one path and the variable it hides
on the other.

\subsubsection{\pyinline{ModuleNotFoundError} and \pyinline{ImportError}}

Importing a module outside $\dom(\mathcal{M})$ is ill-formed (rule \ruleName{module}), as is from-importing a
name that is neither a member nor a submodule of the source module (rules \ruleName{imp-member},
\ruleName{imp-module}); Python raises \pyinline{ModuleNotFoundError} and \pyinline{ImportError} respectively.
Cyclic imports are rejected outright (rule \ruleName{program}); in Python a cycle may run or raise
\pyinline{ImportError}, depending on the order in which the participants access one another's members.

\subsubsection{\pyinline{AttributeError}}
\label{sec:attribute-error}

A module attribute reference resolves only to a member (rules \ruleName{attr-module}, \ruleName{qualified})
or to a submodule among the referring module's dependencies (rule \ruleName{qualified-submodule}); Python
raises \pyinline{AttributeError} when the attribute is absent, and whether a submodule attribute is present
depends on global load state. One case is not ruled out: a member
that is not definitely assigned (bound in only some branches of a top-level conditional) may still be
referenced by attribute, and Python raises \pyinline{AttributeError} when it is unset.

\subsubsection{\pyinline{TypeError}}

A constructor must supply exactly the class's fields, with keywords matching the trailing fields (rule
\ruleName{constr}); Python's generated \pyinline{__init__} raises \pyinline{TypeError} on arity or keyword
mismatch. A class pattern with more positional sub-patterns than the class has fields is likewise a Python
\pyinline{TypeError} and is ill-formed (\figref{well-formed-pat}); a pattern with fewer is permitted by
Python and appears under unsupported features instead.
\end{AddedBlock}
...
\Replaced{Module and program evaluation are given in \figref{eval-modules}; importing names from a loaded module in
\figref{eval-imports}. The loading judgement $q \Rightarrow_{\mathsf{load}} \rho$ evaluates the import prefix
of $q$ and then its statements, mirroring the well-formedness judgement $\vdash_{\mathcal{M}} q$; the mutual
recursion between loading and evaluation is grounded by the acyclicity of $G_{\mathcal{M}}$. Loading is
deterministic; write $\rho_q$ for the unique $\rho$ such that $q \Rightarrow_{\mathsf{load}} \rho$. Since
...
cache of loaded modules.}{Program and module evaluation are given in \figref{eval-modules}, with module loading and importing in a separate figure.} These rules use the environment $\rho_{\pymath{builtins}}$ binding the names of
\figref{predefined}.
