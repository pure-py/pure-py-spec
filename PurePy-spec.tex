...
to be acyclic. \Added{The \emph{dependencies} $\deps(q)$ of a module $q$ are the modules reachable from
$q$ in $G_{\mathcal{M}}$. Since imports precede statements, when a \PurePy program runs under Python, every
module in $\deps(q)$ is loaded before any statement of $q$ runs.}
...
\Added{A submodule entry of $\members(q')$ is accessible from another module $q_0$ (rule \ruleName{qualified-submodule}) only when the submodule is among the dependencies $\deps(q_0)$ of the accessing module: only then is it guaranteed to be loaded. Python raises \pyinline{AttributeError} when the attribute path of an unloaded submodule is accessed.}
