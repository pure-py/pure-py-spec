...
A \PurePy execution context (or \emph{program}) $\langle E, \mathcal{M} \rangle$ consists of a finite,
\emph{prefix-closed} map $\mathcal{M}$ from \emph{fully-qualified names} $M$ (\figref{syntax}) to blocks (the
\emph{modules})---i.e., if $M \in \dom(\mathcal{M})$ then $\parents(M) \subseteq \dom(\mathcal{M})$---together
with a designated \emph{entry point} $E \in \dom(\mathcal{M})$. For each $(M \mapsto b) \in
