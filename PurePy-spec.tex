...
\item \emph{class entry} $C$, of the form $\clsEntry{\Gamma}{q}{\vec{x}}{c}$, recording the declaring context $\Gamma$, defining module $q$, sequence of distinct field names $\vec{x}$, and base class $c$ (or $\bot$ for no base).
\end{itemize}
\end{definition}
...
\set{\bind{\text{\pyinline{__name__}}}{\statusDefAssigned}}$, where $\Gamma_{\textup{\pyinline{builtins}}}$ binds the
...
Suppose $\Gamma_q^0 \vdash \mathcal{M}(q) : \Delta$. Define the \emph{exports} of $q$ as $\Gamma_q = \set{\bind{\text{\pyinline{__name__}}}{\statusDefAssigned}} \runion \Delta$.
\end{definition}
...
exit code $n$, with $0$ denoting successful termination. For any $(\bind{q}{b}) \in \mathcal{M}$, evaluation of
$b$ begins under the initial environment $\rho_q^0 = \rho_{\textup{\pyinline{builtins}}} \runion \set{\bind{\text{\pyinline{__name__}}}{v}}$, where $v$ is \pyinline{"__main__"} if $q = q_{\mathsf{e}}$ and the string literal corresponding to $q$ otherwise.
