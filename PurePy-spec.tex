...
\Added{Rule \ruleName{qualified-submodule} requires the submodule to be among the dependencies $\deps(q_0)$ of the referring module $q_0$, ensuring it is loaded wherever the reference can be evaluated (\S\ref{sec:ruled-out}).}
...
\Added{Definite assignment thus gives each reference a unique lexical referent: a variable is introduced by
...
A well-formed \PurePy program avoids the classes of Python runtime error listed in \figref{prevented-errors}
by construction. (The previous section is the complementary list: programs Python runs that \PurePy rejects.)
Errors from ill-typed operations, such as wrong operand types, calling a non-function, or attribute access on
an object lacking the attribute, are outside the scope of well-formedness.

\begin{figure}[H]
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.26\textwidth}>{\raggedright\arraybackslash}p{0.48\textwidth}>{\raggedright\arraybackslash}p{0.16\textwidth}}
\hline
\textbf{Python error} & \textbf{Notes} & \textbf{See} \\
\hline
\pyinline{NameError}, \pyinline{UnboundLocalError} & Definite assignment enforces a lexical scoping discipline, which happens to rule out entirely unassigned variables (\S\ref{sec:assignment-as-definition}). & \figref{well-formed}, \figref{well-formed-expr} \\[1mm]
\pyinline{ModuleNotFoundError} & An import requires its target to be a module of the program. & \figref{well-formed-module} \\[1mm]
\pyinline{ImportError} & A from-import requires each imported name to be a member of the source module. Import cycles are rejected; a Python cycle may run or fail, depending on the order in which the participants access one another's members. & \figref{imports}, \figref{well-formed-module} \\[1mm]
\pyinline{AttributeError} & A module attribute reference must resolve to a member, or to a submodule among the referring module's dependencies. Not prevented for a member that is not definitely assigned; Python raises \pyinline{AttributeError} when it is unset. & \figref{well-formed-expr} \\[1mm]
\pyinline{TypeError} & A constructor must supply exactly the class's fields, and a class pattern must match the class's arity. A pattern with fewer positional sub-patterns than fields is permitted by Python and unsupported (\S\ref{sec:unsupported}) rather than prevented. & \figref{well-formed-expr}, \figref{well-formed-pat} \\
\hline
\end{tabular}
