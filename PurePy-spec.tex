...
\Added{Rule \ruleName{qualified-submodule} requires the submodule to be among the dependencies $\deps(q_0)$ of the referring module $q_0$. This ensures it is loaded wherever the reference can be evaluated; Python raises \pyinline{AttributeError} on the attribute path of an unloaded submodule.}
...
\begin{AddedBlock}
\subsubsection{A variable and a submodule with the same name}
\label{sec:submodule-clash}

In Python, an attribute of a package can be bound in two ways: by an assignment in the package's own body,
and by a load of a same-named submodule, since loading \pyinline{pkg.sub} assigns the new module to attribute
\pyinline{sub} of \pyinline{pkg}. Which binding is visible depends on which loads have executed. Suppose the
body of \pyinline{pkg} is the single assignment \pyinline{sub = 1}, and \pyinline{pkg} also has a submodule
\pyinline{pkg.sub}. The following two programs are identical except for their imports:
\vspace{-\medskipamount}
...
On the left, \pyinline{pkg.sub} is never loaded, so the assignment in \pyinline{pkg} is visible. On the
right, \pyinline{pkg.sub} is loaded after \pyinline{pkg}, and the load overwrites the assignment. The same
attribute can even change meaning during a run, when a later import loads the submodule. So the name
\pyinline{pkg.sub} has no load-order-independent meaning, and \PurePy rejects the clash: the
\ruleName{module} rule prohibits a module from binding (by import or by assignment) a name that is also the
name of one of its submodules.
\end{AddedBlock}
