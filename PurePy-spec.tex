...
\subsection{Class contexts}

Class contexts $\Lambda$ are finite maps from class names to pairs $(\vec{x}, C)$, where $\vec{x}$ is a sequence of distinct
field names (in declaration order) and $C$ names the base class (or $\bot$ for no base). Metavariables $C$ and $B$ range over class names; $D$ ranges over \emph{fully-qualified class names} of the form $M.C$.
