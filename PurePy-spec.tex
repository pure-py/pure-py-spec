...
\begin{itemize}
\item \emph{definite assignment} status $a \in \B = \set{\statusDefAssigned, \statusNotDefAssigned}$;
\item \emph{module reference}: a fully-qualified module name $q$;
\item \emph{class entry}: a pair $(\vec{x}, c)$ of a sequence of distinct field names and a base class $c$ (or $\bot$ for no base).
\end{itemize}
\end{definition}

\noindent Class entries cover both locally-declared and imported classes; module references arise from import statements. The definite-assignment terminology is borrowed from Java~\cite[\S16]{gosling2025}; $\statusDefAssigned$ indicates \emph{definitely assigned} and $\statusNotDefAssigned$ \emph{not definitely assigned} (or, equivalently, degenerate types ``any'' and ``none'').
It is useful to represent contexts as total functions on the set of all variables by extending the codomain with $\bot$ to indicate \emph{undefined}. We write $\Gamma, x : \theta$ for the context that sends $x$ to $\theta$ and otherwise agrees with $\Gamma$, define $\dom(\Gamma) = \set{x \mid \Gamma(x) \neq \bot}$, and write $\varnothing$ for the context where all variables are undefined. For a set of variables $X$, write $\statusDefAssigned(X)$ for the context $\set{x : \statusDefAssigned \mid x \in X}$; similarly $\statusNotDefAssigned(X)$.
...
A \PurePy \emph{program} $\langle E, \mathcal{q} \rangle$ consists of a finite map
$\mathcal{q}$ from \emph{fully-qualified module names} $q$ (\figref{syntax-stmt}) to \emph{modules} $m$ (each a sequence
of top-level statements $t_1 \ldots t_{n+1}$), together with a designated \emph{entry point} $E \in
\dom(\mathcal{q})$. $\mathcal{q}$ always includes the predefined modules of \figref{predefined} and is prefix-closed:
if $q \in \dom(\mathcal{q})$ then $\parents(q) \subseteq \dom(\mathcal{q})$. The pair $\langle E, \mathcal{q} \rangle$
...
For any $q \in \dom(\mathcal{q})$, the \emph{initial variable context} of $q$ is $\Gamma_q^0 = \Gamma_{\textup{\pyinline{builtins}}} \runion
...
\hline
\end{tabular}
\caption{Predefined modules guaranteed to be in $\mathcal{q}$}
...
The \emph{dependency graph} $G_{\mathcal{q}}$ of a module map has an edge $(q, q')$ iff
$q' \in \imports(\mathcal{q}(q)) \cup \parents(q)$. Program well-formedness (\figref{well-formed-module}) requires the entry module to be well-formed (which recursively requires its imports to be well-formed) and $G_{\mathcal{q}}$ to be acyclic.
...
= e} definitely assigns $\set{\pyinline{x} : \statusDefAssigned}$. \emph{Result types} $R$ classify these outcomes: $R$ is either $\Returns$ (definitely returns) or $\Assigns{\Delta}$ (definitely assigns $\Delta$). Block well-formedness is given in
...
Metavariable $d$ ranges over \emph{fully-qualified} class names of the form $q.c$.
...
\item variables $x$ bound to \emph{module references} $q$, fully-qualified module names.
\end{itemize}
\end{definition}
...
$G_{\Lambda_q}$ has an edge $(c, c')$ iff $\Lambda_q(c) = (d(\vec{x}), c')$ for some $d, \vec{x}$.
\end{definition}
...
For a well-formed module $q$, i.e.\ where $\Gamma_q^0; \varnothing \vdash \mathcal{q}(q) : \Delta; \Lambda$ for some $\Delta, \Lambda$, the \emph{variable context} of $q$ is $\Gamma_q = \Gamma_q^0 \runion \Delta$ and the \emph{local context} of $q$ is $\Lambda_q = \Lambda$.
\end{definition}
...
exit code $n$, with $0$ denoting successful termination. For any $(q \mapsto b) \in \mathcal{q}$, evaluation of
$b$ begins under the initial environment $\rho_q = \rho_{\textup{\pyinline{builtins}}} \runion \set{\text{\pyinline{__name__}}
\mapsto v}$, where $v$ is \pyinline{"__main__"} if $q = E$ and the string literal corresponding to $q$ otherwise.
