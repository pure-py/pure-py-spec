...
\item \emph{module reference} $\modEntry{q}$, for a fully-qualified module name $q$;
\item \emph{class entry} $\clsEntry{\Gamma}{q}{\vec{x}}{c}$, recording the declaring context $\Gamma$, defining module $q$, sequence of distinct field names $\vec{x}$, and base class $c$ (or $\bot$ for no base).
\end{itemize}
\end{definition}
...
$\mathcal{M}$ from \emph{fully-qualified module names} $q$ (\figref{syntax-stmt}) to \emph{modules} $m$ (each a sequence
...
if $q \in \dom(\mathcal{M})$ then $\parents(q) \subseteq \dom(\mathcal{M})$.\footnote{A source tree encodes such an $\mathcal{M}$ by mapping each leaf module to a \pyinline{.py} file and each package to an \pyinline{__init__.py}; prefix-closure then requires an \pyinline{__init__.py} for every enclosing package.} The pair $\langle E, \mathcal{M} \rangle$
...
For any $q \in \dom(\mathcal{M})$, the \emph{initial variable context} of $q$ is $\Gamma_q^0 = \Gamma_{\textup{\pyinline{builtins}}} \runion
\set{\text{\pyinline{__name__}} : \statusDefAssigned}$, where $\Gamma_{\textup{\pyinline{builtins}}}$ binds the
...
\pyinline{math} & \textit{(members specified in a later revision)} \\
...
The \emph{dependency graph} $G_{\mathcal{M}}$ of a module map has an edge $(q, q')$ iff
$q' \in \imports(\mathcal{M}(q)) \cup \parents(q)$. Program well-formedness (\figref{well-formed-module}) requires the entry module to be well-formed (which recursively requires its imports to be well-formed) and $G_{\mathcal{M}}$ to be acyclic.
...
\begin{definition}[Exports of a well-formed module]
For a well-formed module $q$, i.e.\ where $\Gamma_q^0 \vdash \mathcal{M}(q) : \Delta$ for some $\Delta$, the \emph{exports} of $q$ are $\Gamma_q = \set{\text{\pyinline{__name__}} : \statusDefAssigned} \runion \Delta$.
\end{definition}

\noindent Exports comprise the classes and variables a module defines, together with its intrinsic \pyinline{__name__}, never the names it imports: following PEP 484,\footnote{\url{https://peps.python.org/pep-0484/}} an imported name is a private dependency, not re-exported as part of the interface. In particular a module never exports a module, since module names enter scope only via imports.
...
A module is evaluated only after it has been judged well-formed (\figref{well-formed-module}) in a context $\Gamma_q$. Its runtime environment $\rho_q$ is well-formed under $\Gamma_q$ (\figref{well-formed-env}): each binding is the runtime counterpart of its context entry.
...
Program and module evaluation are given in \figref{eval-modules}, with module loading and importing in
\figref{eval-load}. A program evaluates to a non-negative integer
exit code $n$, with $0$ denoting successful termination. For any $(q \mapsto b) \in \mathcal{M}$, evaluation of
$b$ begins under the initial environment $\rho_q^0 = \rho_{\textup{\pyinline{builtins}}} \runion \set{\text{\pyinline{__name__}}
