...
$\mathcal{M}$ from \emph{fully-qualified names} $M$ (\figref{syntax}) to blocks (the \emph{modules}), together
with a designated \emph{entry point} $E \in \dom(\mathcal{M})$. The map $\mathcal{M}$ is prefix-closed: if $M
\in \dom(\mathcal{M})$ then $\parents(M) \subseteq \dom(\mathcal{M})$. For each $(M \mapsto b) \in
