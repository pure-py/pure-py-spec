...
For any $(M \mapsto b) \in \mathcal{M}$, $b$ is checked under the initial variable context $\Gamma_M = \Gamma_{\textup{\pyinline{builtins}}} \runion
\set{\text{\pyinline{__name__}} \mapsto \statusDefAssigned}$, with $\Gamma_{\textup{\pyinline{builtins}}}$ binding the
names of the predefined module \textup{\pyinline{builtins}} (\figref{predefined}). For a well-formed module $M$, the \emph{local context $\Lambda_M$} is the local context produced as an output of $M$'s body well-formedness (\figref{well-formed-module}).
\end{definition}
...
module in $\mathcal{M}$ to be well-formed in its initial context and $G_{\mathcal{M}}$ to be acyclic.
