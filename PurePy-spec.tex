...
\paragraph{Predefined names.}

Every \PurePy module has implicit access to the predefined names of \figref{predefined}. \emph{Builtins} are
provided by \PurePy and are available in every scope without explicit \pyinline{import}. \emph{Module-local}
names are set per module by the import machinery; \pyinline{__name__} has the value \pyinline{"__main__"} in
the entry-point module and the module's own name otherwise.
...
\hline
Builtins & \pyinline{print}, \pyinline{len}, \pyinline{range}, \pyinline{type}, \pyinline{True}, \pyinline{False}, \pyinline{None} \\
