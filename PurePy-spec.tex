...
A \PurePy \emph{program} is a finite map
$\mathcal{M}$ from \emph{fully-qualified module names} (\figref{syntax-stmt}) to \emph{modules} $m$, the \emph{entry module} being the one under the distinguished name $\pymath{\_\_main\_\_} \in \dom(\mathcal{M})$. $\mathcal{M}$ always includes the predefined modules of \figref{predefined} and is prefix-closed:
...
The metafunction $\assignsS$ extends from individual statements to blocks and module bodies by union, and $\imports$ to module bodies. The metafunction $\parents$ collects the proper prefixes of a
fully-qualified name, and $\nameOf(q)$ is the string literal naming $q$. Each module body $\mathcal{M}(q)$ begins with the assignment $\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}$, so $\mathcal{M}(\pymath{\_\_main\_\_})$ binds $\pymath{\_\_name\_\_}$ to $\pymath{"\_\_main\_\_"}$.
...
Suppose $\varnothing \vdash \mathcal{M}(q) : \Delta$. Define the \emph{exports} of $q$ as $\Gamma_q = \Delta$.
\end{definition}
...
\item Local imports: an \pyinline{import} or \pyinline{from} statement inside a function or other block. \PurePy permits imports only at module level; Python allows them anywhere.
...
\figref{eval-load}. These use the environment $\rho_{\pymath{builtins}}$ binding the names of
\figref{predefined}.
