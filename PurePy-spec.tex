...
\hline
\pyinline{NameError}, \pyinline{UnboundLocalError} & \ruleName{var}; branch merge in \ruleName{if} and \ruleName{match} (\figref{well-formed}) & Definite assignment ensures a unique lexical referent, which is stricter (\S\ref{sec:assignment-as-definition}). \\
\pyinline{ModuleNotFoundError} & \ruleName{import}, \ruleName{from-import} (premise $\vdash_{\mathcal{M}} q'$) & \\
\pyinline{ImportError} & \ruleName{imp-member}; \ruleName{program} ($G_{\mathcal{M}}$ acyclic) & A Python import cycle may run or fail, depending on the order in which the participants access one another's members. \\
\pyinline{AttributeError} & \ruleName{qualified}, \ruleName{qualified-submodule}, \ruleName{attr-module} & Not prevented for a member that is not definitely assigned; Python raises \pyinline{AttributeError} when it is unset. \\
