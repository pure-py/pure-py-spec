...
A module is evaluated only after it has been judged well-formed (\figref{well-formed-module}) in a context $\Gamma_q$. Its runtime environment $\rho_q$ is well-formed under $\Gamma_q$ (\figref{well-formed-env}): each binding is the runtime counterpart of its context entry. $\fields$ extends to $\rho_q$ in the obvious way.
...
Block evaluation $\Gamma, \rho, b \Rightarrow r$ is given in \figref{eval}. Function definitions delimit the scope of
sequential-composition effects, so as black boxes functions are always pure.

\input{fig/eval}

Expression evaluation $\Gamma, \rho, e \Rightarrow v$ is given in \figref{eval-expr}. The pattern-matching judgement
