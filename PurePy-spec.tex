...
(\figref{syntax-stmt}) to \emph{module bodies} $m$, of which one is a distinguished \emph{main module} named
$\pymath{\_\_main\_\_}$; here $\nameOf(q)$ is the string literal naming $q$, so
$\nameOf(\pymath{\_\_main\_\_}) = \pymath{"\_\_main\_\_"}$. The well-formedness and evaluation rules check and
evaluate each body $\mathcal{M}(q)$ with the assignment $\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}$
prepended, modelling the $\pymath{\_\_name\_\_}$ binding the runtime supplies automatically rather than one
written in the source. $\mathcal{M}$ additionally always includes the predefined modules of
...
Suppose $\varnothing \vdash (\exAssign{\pymath{\_\_name\_\_}}{\nameOf(q)}) \cons \mathcal{M}(q) : \Delta$. Define the \emph{exports} of $q$ as $\Gamma_q = \Delta$.
\end{definition}
