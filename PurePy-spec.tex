...
$\vec{\iota}$ followed by a sequence of statements $\vec{s}$; since imports are not statements, they cannot
appear in blocks, and hence not in function bodies or after the first non-import statement.} The
...
\figref{eval-expr})}, modelling the $\pymath{\_\_name\_\_}$ binding the runtime supplies automatically rather
than one written in the source. $\mathcal{M}$ additionally always includes the predefined modules of
...
The metafunction $\assignsS$ extends from individual statements to blocks and module bodies by union, and $\imports$ \Replaced{from individual imports to module bodies by union over the import prefix}{to module bodies}. The metafunction $\parents$ collects the proper prefixes of a
