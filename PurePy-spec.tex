...
A \PurePy module is the contents of a single source file. Well-formedness of a module is checked under an
initial context $\Gamma$ in which the names of \figref{predefined} are bound at status $\statusDefAssigned$.
Of these, the names of the predefined module \pyinline{builtins} are bound in every \PurePy module without
an explicit \pyinline{import}; the variable \pyinline{__name__} is bound separately per module, taking the
value \pyinline{"__main__"} in the entry-point module and the module's own name otherwise.
