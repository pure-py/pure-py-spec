...
A \PurePy \emph{program} $\langle q, \mathcal{M} \rangle$ consists of a finite map
$\mathcal{M}$ from \emph{fully-qualified module names} (\figref{syntax-stmt}) to \emph{modules} $m$ (each a sequence
of top-level statements $t_1 \ldots t_{n+1}$), together with a designated \emph{entry point} $q \in
\dom(\mathcal{M})$. $\mathcal{M}$ always includes the predefined modules of \figref{predefined} and is prefix-closed:
the parents of any module in $\dom(\mathcal{M})$ are themselves in $\dom(\mathcal{M})$.\footnote{A source tree encodes such an $\mathcal{M}$ by mapping each leaf module to a \pyinline{.py} file and each package to an \pyinline{__init__.py}; prefix-closure then requires an \pyinline{__init__.py} for every enclosing package.} The map $\mathcal{M}$
...
For any $q \in \dom(\mathcal{M})$, the \emph{initial variable context} of $q$ is $\Gamma_q^0 = \Gamma_{\pymath{builtins}} \runion
\set{\bind{\pymath{\_\_name\_\_}}{\statusDefAssigned}}$, where $\Gamma_{\pymath{builtins}}$ binds the
...
Suppose $\Gamma_q^0 \vdash \mathcal{M}(q) : \Delta$. Define the \emph{exports} of $q$ as $\Gamma_q = \set{\bind{\pymath{\_\_name\_\_}}{\statusDefAssigned}} \runion \Delta$.
\end{definition}
...
\figref{eval-load}. A module body is evaluated under the initial environment $\rho_q^0 = \rho_{\pymath{builtins}}
\runion \set{\bind{\pymath{\_\_name\_\_}}{v}}$, where $v$ is the string literal naming $q$, except that the entry
module binds \pyinline{__name__} to \pyinline{"__main__"}. The environment $\rho_{\pymath{builtins}}$ binds the
names listed for \pyinline{builtins} in \figref{predefined}.
