...
that invite confusion are tightened or excluded, even where doing so departs from strict Python compatibility
at the margins.
...
\item Cross-bracket sequence pattern matching: \pyinline{case [a, b]:} only matches lists, and \pyinline{case (a, b):} only matches tuples. Python admits either pattern against either kind of sequence; \PurePy keeps them type-strict (the bracket form is significant).
\end{itemize}
