...
A context $\Gamma$ (or $\Delta$) is a finite map from variables to one of:
\begin{itemize}
\item a \emph{definite assignment} status $a \in \B = \set{\statusDefAssigned, \statusNotDefAssigned}$, with $\statusDefAssigned$ indicating \emph{definitely assigned} and $\statusNotDefAssigned$ \emph{not definitely assigned};
\item a \emph{module reference}: a fully-qualified module name $M$;
\item a \emph{class entry}: a pair $(\vec{x}, c)$ of a sequence of distinct field names in declaration order and a base class $c$ (or $\bot$ for no base).
\end{itemize}
\end{definition}

\noindent Class entries cover both locally-declared and imported classes; module references arise from import statements. The definite-assignment terminology is borrowed from Java~\cite[\S16]{gosling2025}; one can also think of $\statusDefAssigned$ and $\statusNotDefAssigned$ as degenerate types ``any'' and ``none''. Result types $R$ are either $\Returns$ or $\Assigns{\Delta}$ for some $\Delta$.

It is useful to represent contexts as total functions on the set of all variables by extending the codomain with $\bot$ to indicate \emph{undefined}. We write $\Gamma, x : v$ for the context that sends $x$ to $v$ and otherwise agrees with $\Gamma$, define $\dom(\Gamma) = \set{x \mid \Gamma(x) \neq \bot}$, and write $\varnothing$ for the context where all variables are undefined. For a set of variables $X$, write $\statusDefAssigned(X)$ for the context $\set{x : \statusDefAssigned \mid x \in X}$; similarly $\statusNotDefAssigned(X)$.
...
distributivity does not hold. Both operations are defined pointwise; on $\B_{\bot}$:
%
...
& & a \merge a' &= a \meet a'
\end{align*}}
For $v \in \V \setminus \B$ (module reference or class entry): $\bot \runion v = v \runion \bot = v$ and $v \runion v' = v'$ for any $v' \in \V$. Merge is undefined when either argument is in $\V \setminus \B$, and never invoked at such arguments in practice (only assignments contribute to branching contexts).
\end{definition}

\noindent The $\runion$ operation is right-biased and has $\bot$ as left and right unit. The $\merge$
operation lacks a unit; when exactly one argument is defined and in $\B$, the result is $\statusNotDefAssigned$; when
both are defined and in $\B$, it returns the conjunction of their values in $\B$.
...
\begin{definition}[Initial variable context]
For any $M \in \dom(\mathcal{M})$, the \emph{initial variable context} of $M$ is $\Gamma_M^0 = \Gamma_{\textup{\pyinline{builtins}}} \runion
\set{\text{\pyinline{__name__}} \mapsto \statusDefAssigned}$, where $\Gamma_{\textup{\pyinline{builtins}}}$ binds the
names of the predefined module \textup{\pyinline{builtins}} (\figref{predefined}).
\end{definition}
...
\paragraph{Class/module contexts.}

Metavariable $d$ ranges over \emph{fully-qualified} class names of the form $M.c$.

\begin{definition}[Class/module context]
A class/module context $\Lambda$ is a finite map whose entries are either:
\begin{itemize}
\item class names $c$ bound to \emph{class entries} $(d(\vec{x}), c')$, where $\vec{x}$ is a sequence of distinct field names in declaration order and $c'$ names the base class (or $\bot$ for no base);
\item variables $x$ bound to \emph{module references} $M$, fully-qualified module names.
\end{itemize}
\end{definition}
...
$G_{\Lambda_M}$ has an edge $(c, c')$ iff $\Lambda_M(c) = (d(\vec{x}), c')$ for some $d, \vec{x}$.
\end{definition}
...
\begin{definition}[Variable and local contexts of a well-formed module]
For a well-formed module $M$, i.e.\ where $\Gamma_M^0; \varnothing \vdash \mathcal{M}(M) : \Delta; \Lambda$ for some $\Delta, \Lambda$, the \emph{variable context} of $M$ is $\Gamma_M = \Gamma_M^0 \runion \Delta$ and the \emph{local context} of $M$ is $\Lambda_M = \Lambda$.
\end{definition}
