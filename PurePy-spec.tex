...
\dom(\mathcal{M})$. The pair $\langle E, \mathcal{M} \rangle$ is fixed and ambient in all definitions that follow. The map $\mathcal{M}$ is prefix-closed: if $M \in \dom(\mathcal{M})$ then $\parents(M)
...
$\rho_M = \rho_{\pyinline{builtins}} \runion \set{\text{\pyinline{__name__}} \mapsto v}$, where $v$ is
\pyinline{"__main__"} if $M = E$ and the string literal corresponding to $M$ otherwise. The environment $\rho_{\pyinline{builtins}}$ binds
the names listed for \pyinline{builtins} in \figref{predefined} to their values.

\input{fig/eval}

The pattern-matching judgement $p, v \matches \rho$ is a deterministic partial function on $(p,
