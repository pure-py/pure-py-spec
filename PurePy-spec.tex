...
A \PurePy execution context (or \emph{program}) is a finite set of \emph{modules}, each identified by a
\emph{fully-qualified name} $M$ (\figref{syntax}). One module is designated as the \emph{entry point}.
A module $m$ is a block $b$ (\figref{syntax}). Well-formedness of $m$ is checked
under the initial context $\Gamma_m$, which binds at status $\statusDefAssigned$ the names of
...
\emph{parents} (proper prefixes of $M$); the program-level dependency graph used for cycle-checking combines
$\imports$ with $\parents$ edges.
