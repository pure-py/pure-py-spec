...
The grammar of contexts and result types is given in \figref{contexts}. Metavariable $x$ ranges over variables and $f$ over function names. Contexts $\Gamma, \Delta$ are partial functions from sets of variables to a \emph{definite assignment} status
in $\B = \set{\statusDefAssigned, \statusNotDefAssigned}$. $\statusDefAssigned$ indicates
\emph{definitely assigned} and $\statusNotDefAssigned$ indicates \emph{not definitely assigned}. The definite
...
Metavariables $C$ and $B$ range over \emph{simple} class names; $D$ ranges over \emph{fully-qualified} class names of the form $M.C$.

\begin{definition}[Local context]
A local context $\Lambda$ is a finite map whose entries are either:
\begin{itemize}
\item simple class names $C$ bound to \emph{class entries} $(D(\vec{x}), B)$, where $\vec{x}$ is a sequence of distinct field names (in declaration order) and $B$ names the base class (or $\bot$ for no base);
\item simple identifiers $x$ bound to \emph{module references} $M$, fully-qualified module names.
\end{itemize}
\end{definition}

\noindent Class entries cover both locally-declared and imported classes; module references arise from import statements.

\begin{definition}[Inheritance graph]
$G_{\Lambda_M}$ has an edge $(C, B)$ iff $\Lambda_M(C) = (D(\vec{x}), B)$ for some $D, \vec{x}$.
\end{definition}

\subsection{Modules and entry point}

A \PurePy \emph{program} $\langle E, \mathcal{M} \rangle$ consists of a finite map
$\mathcal{M}$ from \emph{fully-qualified module names} $M$ (\figref{syntax-stmt}) to \emph{modules} $m$ (each a sequence
...
Import statements appear only as a contiguous prefix of a module body, before any other statement. This confines module-loading side-effects (currently only divergence or runtime
errors) to a single zone at the start of each module body.

\begin{definition}[Module well-formedness context]
For any $(M \mapsto b) \in \mathcal{M}$, $b$ is checked under the initial variable context $\Gamma_M^0 = \Gamma_{\textup{\pyinline{builtins}}} \runion
\set{\text{\pyinline{__name__}} \mapsto \statusDefAssigned}$, with $\Gamma_{\textup{\pyinline{builtins}}}$ binding the
names of the predefined module \textup{\pyinline{builtins}} (\figref{predefined}). For a well-formed module $M$, the \emph{variable context} $\Gamma_M = \Gamma_M^0 \runion \Delta$ and \emph{local context} $\Lambda_M$ are the contexts produced as outputs of $M$'s body well-formedness (\figref{well-formed-module}).
\end{definition}
...
\hline
\textup{\pyinline{builtins}} & \pyinline{True}, \pyinline{False}, \pyinline{None}, \pyinline{int}, \pyinline{float}, \pyinline{str}, \pyinline{bool}, \pyinline{list}, \pyinline{tuple}, \pyinline{set}, \pyinline{dict}, \pyinline{abs}, \pyinline{round}, \pyinline{len}, \pyinline{min}, \pyinline{max}, \pyinline{sum}, \pyinline{all}, \pyinline{any}, \pyinline{sorted}, \pyinline{reversed}, \pyinline{range}, \pyinline{enumerate}, \pyinline{zip}, \pyinline{map}, \pyinline{filter}, \pyinline{type}, \pyinline{isinstance} \\
\pyinline{math} & \pyinline{pi}, \pyinline{e}, \pyinline{sqrt}, \pyinline{sin}, \pyinline{cos}, \pyinline{tan}, \pyinline{log}, \pyinline{exp}, \pyinline{floor}, \pyinline{ceil} \\
\pyinline{sys} & \pyinline{argv}, \pyinline{exit}, \pyinline{stdin}, \pyinline{stdout} \\
\pyinline{typing} & \pyinline{Any} \\
\pyinline{dataclasses} & \pyinline{dataclass} \\
\hline
\end{tabular}
\caption{Predefined modules guaranteed to be in $\mathcal{M}$}
\label{fig:predefined}
\end{figure}

The metafunctions $\imports$ and $\assignsS$ extend from individual statements to blocks and module bodies by union. The metafunction $\parents$ collects the proper prefixes of a
fully-qualified name.

\input{fig/modules}

The \emph{dependency graph} $G_{\mathcal{M}}$ of a module map has an edge $(M, M')$ iff
$M' \in \imports(\mathcal{M}(M)) \cup \parents(M)$. Program well-formedness (\figref{well-formed-module}) requires the entry module to be well-formed (which recursively requires its imports to be well-formed) and $G_{\mathcal{M}}$ to be acyclic.
...
Program and module evaluation are given in \figref{eval-modules}. A program evaluates to a non-negative integer
exit code $n$, with $0$ denoting successful termination. For any $(M \mapsto b) \in \mathcal{M}$, evaluation of
$b$ begins under the initial environment $\rho_M = \rho_{\textup{\pyinline{builtins}}} \runion \set{\text{\pyinline{__name__}}
\mapsto v}$, where $v$ is \pyinline{"__main__"} if $M = E$ and the string literal corresponding to $M$ otherwise.
The environment $\rho_{\textup{\pyinline{builtins}}}$ binds the names listed for \textup{\pyinline{builtins}} in \figref{predefined}
to their values.
...
$p, v \matches \rho$ (\figref{match}) is a deterministic partial function; we write $p, v \notmatches$
(with no right-hand side) for $\nexists \rho.\, p, v \matches \rho$, i.e.\ matching is undefined.
