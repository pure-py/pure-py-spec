...
The grammar of contexts and result types is given in \figref{contexts}. The metavariable $x$ ranges over
variables and $f$ over function names. Contexts $\Gamma, \Delta$ are partial functions from sets of variables to a \emph{definite assignment} status
...
Class contexts $\Lambda$ are finite maps from class names to pairs $(F, B)$, where $F$ is a set of field
names and $B$ is either a class name or $\bot$ (no base). The metavariables $C$ and $B$ range over class names.
...
The metafunctions $\imports$, $\classes$ (\figref{modules}), and $\assignsS$ extend from individual statements
to blocks and module bodies by union. The metafunction $\parents$ collects the proper prefixes of a
