...
\versionstring{} of the standard.}: a formal grammar, well-formedness rules based on a
definite-assignment analysis, and an operational semantics that agrees with Python on every run it defines. A reference well-formedness checker decides membership of the subset, and a conformance test suite exercises the
standard against CPython, PyPy, GraalPy, and Pyodide. The guarantee is precise: membership is decidable, and
every run a well-formed \PurePy program has under \PurePy is its Python run.
...
a definite-assignment analysis (\secref{syntax}) and an operational semantics that agrees with Python on
every run it defines (\secref{opsem});
\item a reference well-formedness checker that decides membership of the subset, validated by a conformance test suite
