...
\Added{Every evaluation judgement is implicitly parameterised by a context $\Gamma$ with $\vdash \rho : \Gamma$,
as the figure captions record. A rule passes $\Gamma$ to its premises unchanged, and only the rules that build
a closure, an object or a class entry mention it, storing it in the value so that the value can later be
checked against the context it was created in.}
...
\figref{predefined}. \Replaced{A program evaluates to a \emph{termination kind} (\figref{values}): $\errExit{0}$ if the main module
