...
\Added{A primitive $\primVal{\primFn}$ is a value that can be applied, with $\primFn$ its meaning.
$\mathcal{P}$ binds a member to one where there is no body to evaluate. No rule applies a primitive, so
calling one has no derivation (\issue{132}, \issue{47}).}
