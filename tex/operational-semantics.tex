...
\figref{predefined}. \Replaced{A program evaluates to a \emph{termination kind} (\figref{values}). A run whose main module loads
successfully yields $\errExit{0}$, and one whose main module aborts yields the kind it aborted with.
$\errExit{n}$ is a run that exited with status $n$; the remaining kinds are named after the exceptions the
same program raises under Python. An implementation must agree on which kind a run yields, though how it
reports one is not prescribed, \PurePy having no exceptions of its own.}{A program evaluates to
