...
fails yields the error kind rather than a status. An implementation must agree on the kind and terminate the
run with a non-zero status; how it reports the kind is not prescribed, \PurePy having no exceptions of its
own. The kinds are named after the exceptions the same program raises under Python.}{A program evaluates to
