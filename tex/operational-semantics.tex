...
cache of loaded modules. \Replaced{A module body is checked and evaluated under the context and environment of
$\mathcal{P}(\pyinline{builtins})$, overridden by those its imports produce, so the names of
\figref{predefined} are in scope in every module and an import or an assignment of the same name takes
precedence.}{These rules use the environment $\mathcal{P}(\pyinline{builtins})$ binding the names of
\figref{predefined}.}
\Added{A predefined member is the primitive value $\primVal{q.x}$ (\figref{values}). Calling one has no
derivation: \ruleName{eval-call-nonfun} does not apply to a primitive, and no rule gives one a meaning
(\issue{132}, \issue{47}).} \Replaced{A program evaluates to a \emph{termination kind} (\figref{values}): $\errExit{0}$ if the main module
