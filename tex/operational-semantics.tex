...
$\returns{v}$ represents \emph{abrupt completion} of $s$ with return value $v$, and $\aborts{\kappa}$
abrupt completion \Replaced{that abandons the whole run, carrying the kind $\kappa$}{with an error}.
...
aborts yields the kind rather than a status. An implementation must agree on the kind and terminate the
...
outcome, either $\val{v}$ or $\aborts{\kappa}$ (\figref{values}). The error kinds are designed to
...
have the meanings they have in Python and yield $\aborts{\kappa}$ where Python raises. Where one of these is
