...
\ \PurePy excludes \Replaced{four}{three}. The first is truthiness: the test of an \pyinline{if} or conditional expression, or an
...
\Added{The fourth is in dictionary keys: Python admits any hashable key and treats \pyinline{1},
\pyinline{1.0} and \pyinline{True} as one key, whereas \PurePy admits only strings.}
Only the third is decidable without types, so the \Replaced{others are}{first two are} stated here rather than enforced by
...
Dict comprehensions & \issue{52} \\
Dict literals & \issue{87} \\
