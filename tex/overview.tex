...
\Added{\figref{taxonomy} sets out the cases. The blank cells carry as much as the filled ones. \PurePy accepts
nothing Python rejects, so the left column has a single entry, and a conformant execution never meets a Python
error, which is the guarantee stated above.}

\Added{The cell where \PurePy declines a program Python runs is the boundary of the subset. An implementation
...
\small
\color{addedcolor}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.27\textwidth}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}}
\hline
 & \multicolumn{3}{c}{\textbf{Python}} \\
\textbf{\PurePy} & \textbf{rejects} & \textbf{raises} & \textbf{succeeds} \\
\hline
Not well-formed & Rejected by both & Error prevented (\figref{prevented-errors}) & Excluded by design (\figref{coercions}) \\[1mm]
Well-formed, execution not conformant & & Both fail & Outside the subset (\figref{coercions}) \\[1mm]
Well-formed, execution conformant & & & Same result \\
\hline
\end{tabular}
\caption{How a \PurePy program relates to Python. Blank cells cannot occur}
\label{fig:taxonomy}
\end{figure}
