...
Dictionary keys & \Replaced{Any hashable value}{Any hashable values}, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys\added{; subscript with absent key raises \pyinline{KeyError}, or \pyinline{TypeError} if unhashable} & Strings only & Statically in patterns, dynamically in expressions \\[1mm]
\added{\pyinline{assert} message} & \added{Any object, shown in the traceback} & \added{Strings only} & \added{Dynamically} \\[1mm]
...
dataclass, and a dataclass body declares fields and nothing else, so Python's extension hooks are
unavailable: a class cannot define \pyinline{__getitem__} for subscripting, \pyinline{__iter__} for
iteration, or \pyinline{__eq__} for equality. Subscripting, iteration, equality and membership therefore have
the fixed meanings given in \secref{opsem} rather than dispatching to user code, and the metafunctions that
give those meanings are named after the hooks they stand in for.}{Only dataclasses are supported, so
\pyinline{class} definitions of other kinds are excluded.}
...
wrong number of arguments}, or attribute access on an object lacking the attribute. Each of these has a \Replaced{termination}{error} kind and a rule in the semantics (\figref{eval-fails}).
