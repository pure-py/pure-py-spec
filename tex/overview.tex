...
apply at different levels. Well-formedness is a property of the program, decided statically. Conformance is a
property of a single execution: an execution is conformant when the rules of \secref{opsem} give it a result.
The distinction matters because a program can have more than one execution, and because whether an execution
stays inside \PurePy can depend on the values it computes. Comparing two lists is inside the subset for most
values and outside it when a NaN reaches the comparison (\secref{no-coercions}).}
