...
the run without a derivation, so the program runs under Python but not under \PurePy.
...
\begin{tabular}{>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.29\textwidth}>{\raggedright\arraybackslash}p{0.245\textwidth}>{\raggedright\arraybackslash}p{0.155\textwidth}}
\hline
...
Sequence patterns & List patterns match tuples, and tuple patterns lists & \todo{List patterns match only lists, tuple patterns only tuples} & Neither; results differ (\issue{151}) \\[1mm]
Constructor patterns & Positional class patterns may supply fewer sub-patterns than the class has fields, matching only those given & Require exact arity & Statically \\[1mm]
Dictionary keys & Any hashable values, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys & Strings only & Syntactically in patterns, dynamically in expressions \\[1mm]
...
\pyinline{nan} comparison via container equality & Elements compared by identity before equality, so identical NaNs match but distinct ones do not & Equality undefined & Dynamically \\
\hline
\end{tabular}
