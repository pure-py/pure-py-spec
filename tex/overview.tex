...
Dictionary keys & \Replaced{Any hashable value}{Any hashable values}, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys\added{; a subscript with an absent key raises \pyinline{KeyError}, or \pyinline{TypeError} if it is unhashable} & Strings only\added{, in literals and in subscripts} & Statically in patterns, dynamically in expressions \\[1mm]
...
\Added{Python's extension hooks are excluded with them. A class may not define the special methods by which
...
ill-typed operations, such as wrong operand types, calling a non-function, or attribute access on an object
lacking the attribute. Each of these has an error kind and a rule in the semantics (\figref{eval-fails}).
