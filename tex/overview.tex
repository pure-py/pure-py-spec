...
\Deleted{Under \PurePy a list pattern applied to a tuple fails to match rather than getting stuck, so a
...
Sequence patterns & List patterns match tuples, and tuple patterns lists & List patterns match only lists, tuple patterns only tuples & \replaced{Dynamically}{\issue{151}} \\[1mm]
...
Dictionary keys & \Replaced{Any hashable value}{Any hashable values}, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys & Strings only & Statically in patterns, dynamically in expressions \\[1mm]
