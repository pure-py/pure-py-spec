...
\Added{\PurePy is required to be a genuine subset of Python: a well-formed program is valid Python, and where
\PurePy gives it a meaning, that meaning is the one Python gives it. Two judgements decide this, and they
apply at different levels. A program is well-formed when it passes \PurePy's static checks, which look only
at the program text and always terminate. Conformance is a property of a single execution: an execution is
conformant when the rules of \secref{opsem} give it a result. The distinction matters because a program can
have more than one execution, and because whether an execution stays inside \PurePy can depend on the values
it computes. Comparing two lists is inside the subset for most values and outside it when a NaN reaches the
comparison (\secref{no-coercions}).}

\Added{\figref{taxonomy} sets out the cases. Two of them the picture states by containment rather than by a
label. \PurePy accepts nothing Python rejects, so the well-formed region lies inside the outer one; and a
conformant execution never meets a Python error, so the conformant region lies inside the region Python runs
without error.}

\Added{The region where \PurePy declines a program Python runs is the boundary of the subset. An implementation
may report reaching it, by raising \pyinline{UnsupportedOperationError}. It is not required to, since the
guarantee says nothing about such an execution, and a stock Python interpreter will simply compute Python's
answer.}

\Added{This taxonomy organises the conformance suite (\secref{reference-impl}), and is how each new feature is
classified as \PurePy grows.}

\begin{figure}
\centering
\begin{tikzpicture}[font=\footnotesize, rounded corners=4pt]
  \draw (0,0) rectangle (13.4,7.6);
  \draw (0.9,0.9) rectangle (9.0,6.7);
  \draw[dashed] (4.8,0.45) rectangle (12.95,7.15);
  \draw (5.4,2.2) rectangle (8.4,5.4);
  \node[anchor=north west] at (0.15,7.45) {\textbf{Syntactically valid Python}};
  \node[anchor=north west] at (1.05,6.55) {\textbf{Well-formed \PurePy}};
  \node[anchor=north east, align=right] at (12.8,7.05) {\textbf{Semantically valid Python}\\no runtime error};
  \node[align=center] at (6.9,4.4) {\textbf{Conformant}\\[1mm]Same result as Python};
  \node[align=center] at (2.4,0.45) {Error prevented (\figref{prevented-errors})};
  \node[align=center] at (11.0,3.8) {Excluded by design\\(\figref{coercions})};
  \node[align=center] at (2.85,3.8) {Both fail};
  \node[align=center] at (6.9,6.0) {Outside the subset (\figref{coercions})};
\end{tikzpicture}
