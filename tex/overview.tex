...
\Replaced{Python accepts several implicit coercions and lax matches that are convenient but a recognised source of
error, since what Python accepts rarely coincides with what the author meant.% TODO: citation on implicit-coercion errors
\ \figref{coercions} lists those \PurePy excludes. Only the arity of a constructor pattern is decidable
without types, so the rest are stated here rather than enforced by the well-formedness rules.}{Python accepts several implicit coercions and lax matches that are convenient but a recognised source of error,
...
\hline
Test of \pyinline{if}, conditional expression or \pyinline{assert} & Coerced to \pyinline{bool}. & Must already be boolean. \\[1mm]
Sequence patterns & A list pattern matches a tuple, and a tuple pattern a list. & A list pattern matches only a list, a tuple pattern only a tuple. \\[1mm]
Subscripting & Applies to lists and tuples alike. & Applies to lists alone; a tuple is taken apart by pattern matching. \\[1mm]
Constructor patterns & A positional class pattern may supply fewer sub-patterns than the class has fields, matching only those given. & Requires the exact arity. \\[1mm]
Dictionary keys & Any hashable key, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} one key. & Strings only. \\[1mm]
\pyinline{==} and \pyinline{!=} & Compare operands of any two kinds, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equal. & Compare operands of the same kind only. \\[1mm]
Functions under \pyinline{==} & Compared by identity, so two textually identical lambdas are unequal. & Outside equality. \\
\hline
\end{tabular}
\caption{Python coercions and lax matches that \PurePy excludes}
\label{fig:coercions}
\end{figure}
