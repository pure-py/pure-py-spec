...
\Added{\PurePy is a subset of Python. Every well-formed program is also a valid Python program, and when
\PurePy gives a run a result, Python gives the same one. \figref{taxonomy} shows how the two languages fit
together. Both halves of that promise are visible as nesting: \PurePy accepts nothing that Python rejects,
and a valid \PurePy run never hits a Python error.}

\Added{A program is well-formed when it passes \PurePy's static checks. These look only at the program text
...
\pyinline{UnsupportedOperationError}, but nothing requires it to. The promise covers only the runs that stay
