...
is between sequence kinds\Deleted{ in pattern matching}: Python lets a list pattern match a tuple value, and a tuple
pattern match a list, whereas in \PurePy a list pattern matches only a list and a tuple pattern only a tuple.
\Added{Subscripting draws the same line, applying to lists alone, a tuple being taken apart by pattern
...
\Added{Only the third is decidable without types, so the first two are stated here rather than enforced by
...
Enums & \issue{86} \\
Remaining expression rules & \issue{133} \\
