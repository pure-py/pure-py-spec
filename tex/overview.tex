...
\Replaced{Python accepts several implicit coercions and lax matches. \figref{coercions} lists those \PurePy
...
(\issue{151}).}}{Python accepts several implicit coercions and lax matches that are convenient but a recognised source of error,
since what Python accepts rarely coincides with what the author meant.
\ \PurePy excludes four. The first is truthiness: the test of an \pyinline{if} or conditional expression, or an
...
Subscripts & Apply to lists and tuples alike & Apply to lists alone; tuples are taken apart by pattern matching \\[1mm]
Constructor patterns & Positional class patterns may supply fewer sub-patterns than the class has fields, matching only those given & Require the exact arity \\[1mm]
Dictionary keys & Any hashable values, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys & Strings only \\[1mm]
\pyinline{==} and \pyinline{!=} & Compare operands of any two kinds & Compare operands of the same kind only \\[1mm]
Functions under \pyinline{==} & Compared by identity, so two textually identical lambdas are unequal & Equality undefined \\[1mm]
\pyinline{nan} inside a list, tuple, dict or object & Elements compared by identity before equality, and a NaN is not equal to itself, so containers holding identical NaNs compare equal and those holding distinct ones do not & Equality undefined, though \pyinline{x != x} still detects a NaN \\
\hline
\end{tabular}
