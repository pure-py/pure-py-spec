...
run rather than about the program, because whether an operation is excluded can depend on the values it is
applied to (\secref{no-coercions}). When a run reaches a dynamically excluded operation, an implementation is
free to raise \pyinline{UnsupportedOperationError}, but nothing requires it to. The guarantee covers only
valid runs, and a stock Python interpreter will carry on and give the Python answer. The same classification
organises the conformance suite (\secref{reference-impl}), and it decides where a new feature belongs as
\PurePy grows.}
...
This is why well-formedness rests on a definite-assignment analysis (\secref{syntax}).}
