...
\begin{tikzpicture}[font=\footnotesize, rounded corners=4pt]
  ...
  \draw (5.5,2.6) rectangle (8.5,5.3);
  ...
  \node[align=center] at (2.4,0.85) {\textbf{Ill-formed \PurePy}\\Python would raise (\figref{prevented-errors})};
  \node[align=center] at (11.0,3.9) {\textbf{Ill-formed \PurePy}\\Excluded by design\\(\figref{coercions})};
  \node[align=center] at (3.2,3.9) {Both fail};
  \node[align=center] at (7.0,5.8) {Outside the subset (\figref{coercions})};
  \node[align=center] at (7.0,3.95) {\textbf{Conformant}\\[1mm]Same result as Python};
\end{tikzpicture}
