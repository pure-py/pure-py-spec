...
\todo{Under \PurePy a list pattern applied to a tuple fails to match rather than getting stuck, so a
later case runs and \PurePy gives a different result from Python (\issue{151}).}
...
Sequence patterns & List patterns match tuples, and tuple patterns lists & List patterns match only lists, tuple patterns only tuples & \todo{\issue{151}} \\[1mm]
...
Dictionary keys & \Replaced{Any hashable value}{Any hashable values}, with \pyinline{1}, \pyinline{1.0} and \pyinline{True} equivalent as keys\added{; a subscript with an absent key raises \pyinline{KeyError}, or \pyinline{TypeError} if unhashable} & Strings only & Statically in patterns, dynamically in expressions \\[1mm]
...
A few remaining restrictions do not fit the groups above. \Replaced{A \pyinline{class} definition must be a
dataclass.}{Only dataclasses are supported, so \pyinline{class} definitions of other kinds are excluded.}

\Added{Python's extension hooks are also excluded. A class may not define the special methods by which
Python programs override built-in operations, such as \pyinline{__getitem__} for subscripting,
\pyinline{__iter__} for iteration, or \pyinline{__eq__} for equality, since a dataclass body declares fields
and nothing else. Subscripting, iteration, equality and membership therefore have the fixed meanings given in
\secref{opsem} rather than dispatching to user code, and the metafunctions that give those meanings are named
after the hooks they stand in for.}
...
ill-typed operations, such as wrong operand types, calling a non-function\Added{, calling a function with the
wrong number of arguments}, or attribute access on an object lacking the attribute. Each of these has an error kind and a rule in the semantics (\figref{eval-fails}).
...
\pyinline{AttributeError} & \replaced{The reference is a module attribute, which}{Always: a module attribute reference\ } must resolve to a definitely assigned member.  & \figref{well-formed-expr} \\[1mm]
