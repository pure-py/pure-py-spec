...
\Added{\PurePy is a subset of Python: a well-formed program is valid Python, and where \PurePy gives it a
meaning, that meaning is Python's. \figref{taxonomy} shows how the two languages line up. The nesting in the
figure is the guarantee, since \PurePy accepts nothing Python rejects and a valid \PurePy run never meets a
Python error.}

\Added{A program is well-formed when it passes \PurePy's static checks, which look only at the program text
and always terminate. Semantic validity belongs to a single run rather than to the program, because whether a
run stays inside \PurePy can depend on the values it computes: comparing two lists is inside the subset for
most values and outside it when a NaN reaches the comparison (\secref{no-coercions}).}

\Added{An implementation may report a dynamically excluded operation by raising
\pyinline{UnsupportedOperationError}, but need not. The guarantee says nothing about such a run, and a stock
Python interpreter will compute Python's answer instead.}

\Added{The same classification organises the conformance suite (\secref{reference-impl}) and decides where
each new feature belongs as \PurePy grows.}
