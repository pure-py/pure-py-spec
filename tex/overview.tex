...
\Added{\PurePy is a subset of Python: every well-formed program is a valid Python program, and if \PurePy
gives a run a result, that run produces the same result under Python. \figref{taxonomy} shows the
relationship between the two languages, with the guarantee appearing as containment.}

\Added{Well-formedness is a property of the program, decided by \PurePy's static checks. Semantic validity is
a property of a single run, because whether an operation is excluded can depend on the values it is applied
to (\secref{no-coercions}). An implementation may report a dynamically excluded operation by raising
\pyinline{UnsupportedOperationError}, but is not obliged to. The guarantee covers only valid runs, and a
stock Python interpreter will simply produce the Python result. We use the same classification for the
conformance suite (\secref{reference-impl}) and for placing each new feature as \PurePy grows.}
