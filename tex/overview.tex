...
enforce this are given in \S\ref{sec:captured-vars}.
...
Python performs several implicit coercions that are convenient but a recognised source of error, since the
coerced value rarely coincides with what the author meant.% TODO: citation on implicit-coercion errors
\ \PurePy excludes two. The first is truthiness: the test of an \pyinline{if} or conditional expression, or an
...
\pyinline{pkg} also has a submodule \pyinline{pkg.sub}:
...
definitions of other kinds are excluded. A positional class pattern must supply exactly the class's field
arity, where Python permits fewer.
