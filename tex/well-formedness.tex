...
\Added{Well-formedness decides which programs \PurePy accepts. The block and expression rules rest on a
...
defined, by their principal cases, in \Replaced{\figref{aux}}{\appendixref{sec:aux-syntactic}}. The metafunction $\capturesS$ extends to blocks and
...
\Added{The rule \ruleName{module} checks a module body under the context of $\mathcal{P}(\pyinline{builtins})$,
overridden by the one its imports produce, so the members of \pyinline{builtins} are in scope
unqualified.}
