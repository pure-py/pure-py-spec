...
\Replaced{\PurePy excludes some Python behaviours statically and others only at runtime.
Well-formedness is the static part, a judgement on programs defined in this \seckind{}; the operational
semantics (\secref{opsem}) is defined for well-formed programs.}{Well-formedness decides which programs
\PurePy accepts.} The block and expression rules rest on a definite-assignment analysis; the pattern rules
...
\paperonly{All are defined, by their principal cases, in \figref{aux}.}\speconly{All are defined in
\secref{mf-syntactic}.}
\begin{PaperOnly}
...
The preorder $\Gamma \vdash p \subsumed p'$ (\figref{subsumption}) holds when every value matched by $p$ is also matched by $p'$.
\end{definition}
