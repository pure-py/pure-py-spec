...
\theta \runion \theta' &= \theta' \quad (\text{when } \Replaced{\theta, \theta' \neq \bot}{\theta' \neq \bot})
\end{align*}}
\end{definition}
...
\theta \join \theta' &= \theta' \quad (\text{otherwise, when } \theta, \theta' \neq \bot) \\
\bot \join \theta = \theta \join \bot &= \theta
\end{align*}}
\end{definition}

\noindent Join combines module references for the same module recursively, preferring full references to stubs, and is otherwise right-biased like override. It is used when processing import prefixes, where two imports rooted at the same package must combine rather than shadow; it applies identically to module values (\S\ref{sec:opsem}).
\end{AddedBlock}
