...
The metavariable $\odot$ ranges over the supported binary operators \Added{other than \pyinline{and} and
\pyinline{or},} and $\ominus$ over the supported unary
operators, both enumerated in \figref{precedence}. \Added{The operators \pyinline{and} and \pyinline{or} have their own
...
body $\seq{\iota}\;\seq{s}$ it acts on $\seq{s}$. Write \Replaced{$q \prefixOf q'$}{$p \prefixOf q$} (\emph{prefix}) when \Replaced{$q' = q$ or $q' =
q.q''$ for some $q''$}{$q = p$ or $q =
p.q'$ for some $q'$}, and $\properPrefixOf$ for the strict order; the proper prefixes of a module name
are its \emph{ancestors}. The $\Added{\submods}\Deleted{\submodules}$ metafunction is
...
\figref{well-formed}; expression well-formedness in \figref{well-formed-expr}\Added{, with the compound forms
in \figref{well-formed-expr-compound}}.
