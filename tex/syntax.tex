...
\begin{tabular}{llll}
\hline
\textbf{Module} & \textbf{Member} & \multicolumn{2}{l}{\added{\textbf{Meaning}}} \\
\hline
\textup{\pyinline{builtins}} & \pyinline{print} & & \\
 & \pyinline{len} & \added{$\primFn(v) = \val{n}$} & \added{if $\iterOp(v) = u_1 \cons \ldots \cons u_n$} \\
 & & \added{$\primFn(\seq{u}) = \aborts{\errType}$} & \added{(otherwise)} \\
 & \pyinline{range} & \added{$\primFn(n) = \val{\exList{0, \ldots, n-1}}$} & \added{if $n$ whole number, $n > 0$} \\
 & & \added{$\primFn(n) = \val{\exList{}}$} & \added{if $n$ whole number, $n \leq 0$} \\
...
\pyinline{sys} & \pyinline{argv} & & \\
 & \pyinline{exit} & \added{$\primFn(\varepsilon) = \aborts{\errExit{0}}$} & \\
 & & \added{$\primFn(n) = \aborts{\errExit{n}}$} & \\
