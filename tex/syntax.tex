...
For each predefined module $q$ of \figref{predefined}, the context $\Gamma_q$ binds the members listed for
$q$, together with \pyinline{__name__}, all definitely assigned. The dynamic semantics is parameterised by
an environment $\rho_q$ for each such $q$, binding the same names to implementation-defined values. The
rules \ruleName{predefined} and \ruleName{eval-predefined} read off $\Gamma_q$ and $\rho_q$.
\end{definition}
\end{AddedBlock}
...
\pyinline{math} & \added{\pyinline{pi}, \pyinline{e}, \pyinline{sqrt}, \pyinline{exp}, \pyinline{log}, \pyinline{sin}, \pyinline{cos}, \pyinline{tan}, \pyinline{floor}, \pyinline{ceil}} \\
...
\Replaced{In this section, loading a module means checking its body and computing its member context
(\figref{well-formed-module}); it is the static counterpart of run-time loading (\secref{opsem}). Both
import forms load their target, together with its proper prefixes
...
prefix that is an ancestor of the importing module is exempt from loading first, because at run time such
an ancestor may still be loading. The exemption applies only to from-imports, because for a plain import
the only prefix that could never load first is the importing module itself, and an import naming a proper
descendant of the importing module is prohibited outright (\secref{no-load-dependent}). Loading a module
...
recursively, every module the program can load; a module that is never imported is not checked.}{The \emph{dependency graph}
