...
\figref{predefined}, \Replaced{whose bodies are implementation-defined and whose contexts and environments are given directly (rules \ruleName{predefined} and \ruleName{eval-predefined})}{each defining exactly its listed members}, and is prefix-closed: the parent of any module in $\dom(\mathcal{M})$ is itself in
...
definitions that follow\Added{, as is the name $q$ of the enclosing module in the rules that check or evaluate a single module's body}.
...
For each predefined module $m$ of \figref{predefined}, the context $\Gamma_m$ binds the members listed for
$m$, together with \pyinline{__name__}, all definitely assigned; the environment $\rho_m$ binds the same
names to their implementation-defined values.
\end{definition}
\end{AddedBlock}
...
\Replaced{The metafunction $\assignsS$ extends by union from individual statements to blocks; on a module
body $\seq{\iota}\;\seq{s}$ it acts on $\seq{s}$. Write $p \prefixOf q$ (\emph{prefix}) when $q = p$ or $q =
p.q'$ for some $q'$, and $\properPrefixOf$ for the strict order. The $\submodules$ metafunction is
collected, with the class-related metafunctions, in Appendix~\ref{sec:aux-modules}.}{The metafunction $\assignsS$ extends by union from individual statements to
blocks, and $\imports$ and $\deps$ from individual imports to import sequences; on a module body
$\seq{\iota}\;\seq{s}$, they act on $\seq{s}$ and $\seq{\iota}$ respectively. Write $p \prefixOf q$
(\emph{prefix}) when $q = p$ or $q = p.q'$ for some $q'$, and $\properPrefixOf$ for the strict order. The
module-related metafunctions are collected in Appendix~\ref{sec:aux-modules}.}

\Replaced{An import cannot run until other modules have loaded, and its rules
(\figref{well-formed-imports}) have a load premise for each. Both import forms load their target. A plain
import also loads every proper prefix of the target, linking each module into its parent (the links
judgement), so that the binding for the target's root supports attribute access along the whole chain. A
from-import likewise loads every proper prefix of its target, together with any imported name that denotes
a submodule; excepted are prefixes that enclose the importing module itself, which may still be loading
when the import runs and so cannot be required to load first. A plain import makes no such exception; an
import naming a proper descendant of the importing module, the one case where the exception would be
forced, is prohibited outright (\secref{no-load-dependent}). Program well-formedness
(\figref{well-formed-module}) requires the main module to be well-formed; the load premises then reach
every module the program can load, so a module that is never imported is not checked. The rules are
interpreted inductively, so import cycles are ill-formed: loading a module in a cycle would require that
module's own load derivation as a premise, and no finite derivation exists.}{The \emph{dependency graph}
$G_{\mathcal{M}}$ of a program has an edge $(q, q')$ iff $q' \in \deps_{q}(\mathcal{M}(q))$. Every import
depends on its target's ancestors; excepted are ancestors enclosing the importing module itself, which are
necessarily still in progress when it runs and so cannot be required to load first. The exception appears
only in the from-import clause because an import naming a proper descendant of the importing module is
prohibited outright (\secref{no-load-dependent}). In the rules (\figref{well-formed-imports}), the
exemption is the bound on the loads-to judgement, whose prefixes the walk skips: $\varepsilon$ for an
import, the importing module for a from-import. Program well-formedness (\figref{well-formed-module})
requires the main module to be well-formed (which recursively requires its imports to be well-formed) and
$G_{\mathcal{M}}$ to be acyclic.}
...
defined, by their principal cases, in Appendix~\ref{sec:aux-syntactic}. $\capturesS$ extends to blocks and
...
The preorder $p \subsumed p'$ (Appendix~\ref{sec:aux-subsumption}) holds when every value matched by $p$ is also matched by $p'$.
\end{definition}
...
\noindent The $\Returns \override R$ case never arises because the well-formedness judgement prohibits unreachable statements (\secref{unreachable}).
...
on the \ruleName{seq} rule: no variable captured by a statement $s$ may be reassigned in the rest of the block
...
an assignment of its class name for these definitions, though re-declaring a class name is prohibited outright
