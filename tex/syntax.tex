...
\hline
\textbf{Member} & \multicolumn{2}{l}{\textbf{Meaning}} \\
\hline
\pyinline{len} & $\primFn(v) = \val{n}$ & if $\iterOp(v) = u_1 \cons \ldots \cons u_n$ \\
 & $\primFn(\seq{u}) = \aborts{\errType}$ & (otherwise) \\
\pyinline{range} & $\primFn(n) = \val{\exList{0, \ldots, n-1}}$ & if $n$ whole number, $n > 0$ \\
 & $\primFn(n) = \val{\exList{}}$ & if $n$ whole number, $n \leq 0$ \\
\pyinline{exit} & $\primFn(\varepsilon) = \aborts{\errExit{0}}$ & \\
