...
\theta \runion \theta' &= \theta' \quad (\Replaced{\text{otherwise}}{\text{when } \theta' \neq \bot})
\end{align*}}
\end{definition}
...
{\small
\begin{align*}
\exModF{q}{\Gamma} \join \exModF{q}{\Gamma'} &= \exModF{q}{\Gamma \join \Gamma'} \\
\exMod{q} \join \exModF{q}{\Gamma} = \exModF{q}{\Gamma} \join \exMod{q} &= \exModF{q}{\Gamma} \\
\bot \join \theta = \theta \join \bot &= \theta \\
\theta \join \theta' &= \theta' \quad (\text{otherwise})
\end{align*}}
\end{definition}

\noindent Join combines module references for the same module recursively, preferring full references to stubs, and is otherwise right-biased like override. It is used when processing import prefixes, where two imports rooted at the same package must combine rather than shadow. The same equations, read with a loaded reference $\exModF{q}{\rho}$ in place of a full reference and environments in place of contexts, define join on values, used by the loading rules of \S\ref{sec:opsem}.
\end{AddedBlock}
...
\noindent Rule \ruleName{attr-module} requires the resolved entry to be usable: Python raises \pyinline{AttributeError} both for a member that is not definitely assigned and for a submodule that has not been imported (\figref{prevented-errors}).
\end{AddedBlock}
