...
\noindent Subsumption is used by the well-formedness rule for $\pyinline{match}$ to rule out unreachable cases and to check exhaustiveness; with the current pattern set, the latter reduces to requiring a catch-all (var or wildcard) case. \Added{In \ruleName{sub-constr} a constructor pattern's sub-patterns are written
$\seq{p}$, and $\seq{p}[x]$ is the one supplied for field $x$, whether positionally or by keyword;
