...
becomes abstract syntax follows Python, including the indentation that delimits blocks\Deleted{; say where \PurePy
narrows those rules}.
...
$q$, together with \pyinline{__name__}, to \Replaced{an implementation-defined value}{implementation-defined values}.\Added{ A member given a value in \figref{predefined} is bound to that value.}\Deleted{Its context is whatever types that
...
\begin{tabular}{llll}
\hline
\textbf{Module} & \textbf{Member} & \multicolumn{2}{l}{\added{\textbf{Value}}} \\
\hline
\textup{\pyinline{builtins}} & \pyinline{print} & & \\
 & \pyinline{len} & \multicolumn{2}{l}{\added{$\primVal{\primFn}$ where}} \\
 & & \added{$\quad \primFn(v) = \val{n}$} & \added{if $\iterOp(v) = u_1 \cons \ldots \cons u_n$} \\
 & & \added{$\quad \primFn(\seq{u}) = \aborts{\errType}$} & \added{(otherwise)} \\
 & \pyinline{range} & \multicolumn{2}{l}{\added{$\primVal{\primFn}$ where}} \\
 & & \added{$\quad \primFn(n) = \val{\exList{0, \ldots, n-1}}$} & \added{if $n$ whole number, $n > 0$} \\
 & & \added{$\quad \primFn(n) = \val{\exList{}}$} & \added{if $n$ whole number, $n \leq 0$} \\
...
\pyinline{sys} & \pyinline{argv} & & \\
 & \pyinline{exit} & \multicolumn{2}{l}{\added{$\primVal{\primFn}$ where}} \\
 & & \added{$\quad \primFn(\varepsilon) = \aborts{\errExit{0}}$} & \\
 & & \added{$\quad \primFn(n) = \aborts{\errExit{n}}$} & \\
