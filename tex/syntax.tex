...
$q$, together with \pyinline{__name__}, to \Replaced{an implementation-defined value}{implementation-defined values}.\Added{ A member given a meaning
$\primFn$ in \figref{predefined} is bound to $\primVal{\primFn}$.}\Deleted{Its context is whatever types that
...
\hline
\end{tabular}
\subcaption{Modules and their members}
\end{subfigure}
...
\begin{subfigure}{\textwidth}
\centering
\added{\begin{tabular}{ll}
\hline
\textbf{Member} & \textbf{Meaning} \\
\hline
\pyinline{len} & $\begin{aligned}[t]
  \primFn(v) &= \val{n} && \text{if } \iterOp(v) = u_1 \cons \ldots \cons u_n \\
  \primFn(\seq{u}) &= \aborts{\errType} && (\text{otherwise})
\end{aligned}$ \\[3mm]
\pyinline{range} & $\begin{aligned}[t]
  \primFn(n) &= \val{\exList{0, \ldots, n-1}} && \text{if } n \text{ whole number}, n > 0 \\
  \primFn(n) &= \val{\exList{}} && \text{if } n \text{ whole number}, n \leq 0
\end{aligned}$ \\[3mm]
\pyinline{exit} & $\begin{aligned}[t]
  \primFn(\varepsilon) &= \aborts{\errExit{0}} \\
  \primFn(n) &= \aborts{\errExit{n}}
\end{aligned}$ \\
\hline
\end{tabular}}
\subcaption{\added{Meanings of predefined functions}}
\end{subfigure}
\caption{Predefined modules}
