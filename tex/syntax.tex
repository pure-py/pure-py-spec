...
(\figref{well-formed-imports}). Both import forms load their target, together with its proper prefixes,
the packages that enclose it. A plain import also records each of these packages as a member of its parent
(the loads-as judgement), so that the binding created for the root of the target gives attribute access
along the whole chain. A from-import also loads any imported name that denotes a submodule. A prefix that
is an ancestor of the importing module is exempt from loading first, because such an ancestor may still be
loading when the import runs. The exemption applies only to from-imports, because for a plain import the
only prefix that could never load first is the importing module itself, and an import naming a proper
descendant of the importing module is prohibited outright (\secref{no-load-dependent}). Program
