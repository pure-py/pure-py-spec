...
to be acyclic. The \emph{dependencies} $\deps(q)$ of a module $q$ (\figref{modules}) are the modules
...
\noindent The exports $\Delta$ comprise the classes and variables a module defines, together with its intrinsic \pyinline{__name__}, never the names it imports: following PEP 484\footnote{\url{https://peps.python.org/pep-0484/}}, an imported name is a private dependency, not re-exported as part of the interface. The members $\members(q)$ also contain an entry for each submodule $q.x$; the two parts are disjoint, because the module rules prohibit a module from binding a name that is also the name of one of its submodules (\S\ref{sec:submodule-clash}).

Rule \ruleName{qualified-submodule} requires the submodule to be among the dependencies $\deps(q_0)$ of the referring module $q_0$, ensuring it is loaded wherever the reference can be evaluated (\figref{prevented-errors}).
...
\item Referring to a submodule the referring module does not itself import: \pyinline{import a} followed by \pyinline{a.b.f()} is rejected unless \pyinline{a.b} is among the module's own dependencies. Python permits the reference whenever some earlier import, anywhere in the program, loaded \pyinline{a.b}.
\end{itemize}
