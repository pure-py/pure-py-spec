...
The syntax of \PurePy is given in \Replaced{\figrefThree{syntax-stmt}{syntax-expr}{syntax-types}}{\figrefTwo{syntax-stmt}{syntax-expr}}. A \emph{block} is a non-empty sequence of
...
\emph{right near-semiring}~\cite{vanhoorn1967}: $\override$ right-distributes over $\merge$ (Lemma~\ref{lem:right-distributivity}), but left
...
remain definitely assigned if only some branches assign it\paperonly{ (see also \secref{local-assignment})}.
...
Parameters carry optional annotations $\pi$ and function definitions optional return annotations
$\omega$, while dataclass fields and annotated assignments carry types $\tau$; types and annotations are
given in \figref{syntax-types}. We write $\seq{x}\,\seq{\pi}$ for the parameter list $x_1\pi_1, \ldots,
x_n\pi_n$. The well-formedness rules and operational semantics do not consult annotations, so an
annotated assignment is checked and evaluated as an assignment (\ruleName{ann-assign},
\ruleName{eval-ann-assign}). Annotations are given meaning by the type system.
\end{AddedBlock}
...
$\mathcal{P}$ takes each module $q$ of \figref{predefined} to a pair $(\rho, \Gamma)$ with $\vdash \rho :
\Gamma$ (\figref{well-formed-env}), where $\rho$ binds $\pymath{\_\_name\_\_}$ to $\nameOf(q)$ and each
member listed for $q$ in \figref{predefined} to its value.
\end{definition}
...
Write $q \prefixOf q'$ (\emph{prefix}) when $q' = q$ or $q' = q.q''$ for some $q''$, and $\properPrefixOf$ for
