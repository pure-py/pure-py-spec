...
\item a \emph{class entry} $C$, of the form $\clsEntry{\Gamma}{q\added{.c}}{\seq{x}}{c'}$, recording the declaring context $\Gamma$, \added{the qualified name $q.c$ of the class}\deleted{defining module $q$}, sequence of distinct field names $\seq{x}$, and \deleted{base class $c$}\added{base class name $c'$} (or $\bot$ for no base). \Added{A module declares each class name at most once, so the qualified name is the identity of the class: two entries denote the same class exactly when their qualified names agree.}
\end{itemize}
\end{definition}
