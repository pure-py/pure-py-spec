...
The \emph{dependency graph} $G_{\mathcal{M}}$ \Replaced{of a program}{of a module map} has an edge $(q, q')$ iff $q' \in
...
\Replaced{Suppose $\mathcal{M}(q) = \vec{\iota}\;\vec{s}$ with $\vdash \vec{\iota} : \Gamma$ and $\Gamma \vdash \bodyOf(q) : \Delta$.}{Suppose $\varnothing \vdash \bodyOf(q) : \Delta$.} \Replaced{Define the \emph{exports} of $q$ and the \emph{members} of $q$ as the following functions:}{Define the \emph{exports} of $q$ as $\Delta$, and the \emph{context} of $q$ as $\Gamma_q = \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \Delta$.}
\begin{enumerate}
\item {\color{blue}$\exportsOf(q) = \Delta$;}
\item {\color{blue}$\members(q) = \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \exportsOf(q)$.}
\end{enumerate}
\end{definition}
