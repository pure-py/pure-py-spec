...
An attribute of a module can be missing at runtime in two ways. First, a member bound on only some paths
through the module body is not definitely assigned (its entry has status $\statusNotDefAssigned$), and the
attribute is missing when no binding path ran:
\vspace{-\medskipamount}
