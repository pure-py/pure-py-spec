...
\item a \emph{module reference}, for a fully-qualified module name $q$: either a \emph{stub} $\modStub{q}$, or \emph{loaded} reference $\modLoaded{q}{\Gamma}$, which also carries the member context $\Gamma$ of $q$;
\item a \emph{class entry} $C$, of the form $\clsEntry{\Gamma}{q\added{.c}}{\seq{x}}{c'}$, recording the declaring context $\Gamma$, \added{the qualified name $q.c$ of the class}\deleted{defining module $q$}, sequence of distinct field names $\seq{x}$, and \deleted{base class $c$}\added{base class name $c'$} (or $\bot$ for no base).
\end{itemize}
\end{definition}

\noindent Module references arise from import statements. \Added{A module declares each class name at most once, so the qualified name of a class entry is the identity of the class.} The definite-assignment terminology is borrowed from Java~\cite[\S16]{gosling2025}; $\statusDefAssigned$ indicates \emph{definitely assigned} and $\statusNotDefAssigned$ \emph{not definitely assigned} (or, equivalently, degenerate types ``any'' and ``none'').
...
$\dom(\mathcal{M})$.\footnote{A source tree encodes such an $\mathcal{M}$: each module's body is its \texttt{.py}
...
construction.} The map $\mathcal{M}$ is fixed and ambient in all
...
\hline
\textbf{Module} & \textbf{Guaranteed members} \\
\hline
...
{\color{blue}The members listed for each predefined module are a lower bound: the module defines at least these, and may
define others. An empty entry lists none as guaranteed, but the module may still define some; later versions of
this specification will list more. A member access $q.x$ on a predefined module $q$ is therefore well-formed for
any member name $x$, since $q$ may define it. If $x$ is in fact absent at run time, evaluation is stuck, matching
the \pyinline{AttributeError} Python raises. Bare names resolve only to the guaranteed members of
\pyinline{builtins} ($\Gamma_{\pymath{builtins}}$), so any extra members are reachable only by qualified access.}

The metafunction $\assignsS$ extends by union from individual statements to blocks, and $\imports$ and $\deps$ from individual imports to import sequences; on a module body $\seq{\iota}\;\seq{s}$, they act on $\seq{s}$ and $\seq{\iota}$ respectively.  Write $p \prefixOf q$ (\emph{prefix}) when $q = p$ or $q = p.q'$ for some $q'$, and $\properPrefixOf$ for the strict order.

\input{fig/modules}

The \emph{dependency graph} $G_{\mathcal{M}}$ of a program has an edge $(q, q')$ iff $q' \in
\deps_{q}(\mathcal{M}(q))$.
Every import depends on its target's ancestors; excepted are ancestors enclosing the importing
module itself, which are necessarily still in progress when it runs and so cannot be required to load
first. The exception appears only in the from-import clause because an import naming a proper descendant
of the importing module is prohibited outright (\S\ref{sec:prohibited}). In the rules
(\figref{well-formed-imports}), the exemption is the bound on the loads-to judgement, whose prefixes the walk skips:
$\varepsilon$ for an import, the importing module for a from-import. Program well-formedness (\figref{well-formed-module}) requires the
...
\noindent The $\Returns \override R$ case never arises because the well-formedness judgement prohibits unreachable statements (\S\ref{sec:unreachable}).
...
\Added{A dataclass declaration counts as an assignment of its class name for these definitions, though
re-declaring a class name is prohibited outright (\S\ref{sec:prohibited}).}
...
\item \Added{Re-declaring a class name: a dataclass declaration whose name already denotes a class in the module. Python permits the re-declaration, leaving the earlier class reachable only through its existing instances; \PurePy requires a class name to be unique within its module, so that a qualified name identifies a class.}
...
\pyinline{AttributeError} & A module attribute reference must resolve to a definitely assigned member\deleted{.}\Added{, except on a predefined module, whose members are only lower-bounded (\figref{predefined}): there a reference to an absent member is well-formed but stuck at run time, as in Python.}  & \figref{well-formed-expr} \\[1mm]
