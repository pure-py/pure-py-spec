...
\noindent \Replaced{The output context $\Gamma$ of the module well-formedness judgement (\figref{well-formed-module}) comprises}{The exports $\Delta$ comprise} the classes and variables a module defines, together with its intrinsic \pyinline{__name__} \Added{and a stub for each submodule}, never the names it imports: following PEP 484\footnote{\url{https://peps.python.org/pep-0484/}}, an imported name is a private dependency, not re-exported as part of the interface. \Replaced{Submodule stubs and the module's own bindings are disjoint}{The members $\members(q)$ also contain an entry for each submodule $q.x$; the two parts are disjoint}, because the module rules prohibit a module from binding a name that is also the name of \Replaced{a submodule}{one of its submodules} (\S\ref{sec:submodule-clash}).
