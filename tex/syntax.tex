...
The metafunction $\assignsS$ extends by union from individual statements to blocks, and $\imports$ {\color{blue}and $\deps$} from individual imports to import sequences; on a module body $\seq{\iota}\;\seq{s}$, they act on $\seq{s}$ and $\seq{\iota}$ respectively. The metafunction $\parents$ collects the proper prefixes of a
...
\Added{A bare import depends on its target's ancestors, which it loads; a from-import only on ancestors
outside the importing module's own package, since the enclosing packages are necessarily in progress when
the module runs.} Program well-formedness (\figref{well-formed-module}) requires the
...
\item \Added{A bare import naming a proper descendant of the importing module (\pyinline{import x.y} inside module \pyinline{x}): the binding for the head would denote the still-loading module. Python permits it, binding the partially-initialised module. A from-import of the module's own submodule is permitted.}
