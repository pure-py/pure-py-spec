...
The semantics is parameterised by a context $\Gamma_m$ and an environment $\rho_m$ for each predefined
module $m$ of \figref{predefined}: $\Gamma_m$ binds the members listed for $m$, together with
\pyinline{__name__}, all definitely assigned, and $\rho_m$ binds the same names to implementation-defined
values. The rules \ruleName{predefined} and \ruleName{eval-predefined} read off these parameters.
\end{definition}
\end{AddedBlock}
