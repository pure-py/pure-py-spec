...
\Added{The grammar of \PurePy is an abstract syntax, and its metavariables range over values rather than over
source text: $n$ is a number rather than a numeral, and $w$ is a string rather than a lexeme. So
\pyinline{1} and \pyinline{0x1} are the same literal, as are the two ways of writing a newline as a string,
with an escape sequence or with a character code. How source text becomes abstract syntax follows Python:
identifiers, numeric and string literals with their quoting and escape sequences, comments, and the
indentation that delimits blocks. \todo{Say which of Python's lexical rules \PurePy narrows.}}
