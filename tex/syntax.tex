...
\begin{AddedBlock}
\begin{definition}[Usable entry]
An entry $\theta$ is \emph{usable}, written $\usable(\theta)$, unless it is $\statusNotDefAssigned$ or a stub.
\end{definition}

\noindent Rule \ruleName{attr-module} requires the resolved entry to be usable: an unusable entry is exactly one whose attribute may be missing at runtime, raising \pyinline{AttributeError} (\S\ref{sec:uninitialised-members}, \figref{prevented-errors}).
\end{AddedBlock}
...
In Python, an attribute of a module can be missing at runtime in two ways; \PurePy rejects both
statically. First, a member assigned only in some branches of the module body is not definitely assigned
(its entry has status $\statusNotDefAssigned$); on a run that takes no assigning branch, the module ends up
without the attribute. Here \pyinline{b} is \pyinline{False}, so \pyinline{x} is never assigned:
\vspace{-\medskipamount}
...
Second, a submodule is not an attribute of its package until it is loaded (\S\ref{sec:submodule-clash}), so
the attribute is missing unless some import loaded it:
\vspace{-\medskipamount}
...
Rule \ruleName{attr-module} rejects both references: the first resolves to an entry that is not definitely
assigned, the second to a stub, and neither is usable.
\end{AddedBlock}
...
\pyinline{AttributeError} & {\color{blue}Always: a module attribute reference must resolve to a usable entry: a definitely assigned member, or an imported submodule.} {\color{deletedgray}The attribute is a module member that is definitely assigned, or a submodule among the referring module's dependencies.} & \figref{well-formed-expr} \\[1mm]
