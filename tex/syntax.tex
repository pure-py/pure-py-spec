...
\Replaced{Suppose $\mathcal{M}(q) = \vec{\iota}\;\vec{s}$ with $\vdash \vec{\iota} : \Gamma$ and $\Gamma \vdash \bodyOf(q) : \Delta$.}{Suppose $\varnothing \vdash \bodyOf(q) : \Delta$.} Define:
\begin{enumerate}
\item the \emph{exports} of $q$ as \Added{$\exportsOf(q) =$} $\Delta$;
\item the \Replaced{\emph{members} of $q$ as $\members(q)$}{\emph{context} of $q$ as $\Gamma_q$} $= \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion {\color{blue}\exportsOf(q)}\,{\color{deletedgray}\Delta}$.
\end{enumerate}
\end{definition}
