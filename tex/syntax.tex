...
An attribute of a module can be missing at runtime in two ways. First, a member assigned only in some
branches of the module body is not definitely assigned (its entry has status $\statusNotDefAssigned$); on a
run that takes no assigning branch, the module ends up without the attribute. Here \pyinline{b} is
\pyinline{False}, so \pyinline{x} is never assigned:
\vspace{-\medskipamount}
