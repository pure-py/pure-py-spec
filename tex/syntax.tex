...
\item a \emph{class entry} $C$, of the form $\clsEntry{\Gamma}{q}{\seq{x}}{c}$, recording the declaring context $\Gamma$, defining module $q$, sequence of distinct field names $\seq{x}$, and base class $c$ (or $\bot$ for no base).
\end{itemize}
\end{definition}
...
$\dom(\mathcal{M})$.\footnote{{\color{blue}A source tree encodes such an $\mathcal{M}$: each module's body is its \texttt{.py}
...
construction.}\,{\color{deletedgray}A source tree encodes such an $\mathcal{M}$ by mapping each leaf module to a
...
The metafunction $\assignsS$ extends by union from individual statements to blocks, and $\imports$ {\color{blue}and $\deps$} from individual imports to import sequences; on a module body $\seq{\iota}\;\seq{s}$, they act on $\seq{s}$ and $\seq{\iota}$ respectively. {\color{deletedgray}The metafunction $\parents$ collects the proper prefixes of a
fully-qualified name.} \Added{Write $p \prefixOf q$ (\emph{prefix}) when $q = p$ or $q = p.q'$ for some $q'$, and $\properPrefixOf$ for the strict order.}

\input{fig/modules}

The \emph{dependency graph} $G_{\mathcal{M}}$ of a program has an edge $(q, q')$ iff $q' \in
{\color{blue}\deps_{q}(\mathcal{M}(q))}\,{\color{deletedgray}\imports(\mathcal{M}(q)) \cup \parents(q)}$.
\Added{Every import depends on its target's ancestors; excepted are ancestors enclosing the importing
