...
\Replaced{Suppose $\vdash_{\mathcal{M}} q : \Delta$, where we call $\Delta$ the \emph{exports} of $q$. Define the \emph{members} of $q$ as $\members(q) = \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \Delta$.}{Suppose $\varnothing \vdash \bodyOf(q) : \Delta$. Define the \emph{exports} of $q$ as $\Delta$, and the \emph{context} of $q$ as $\Gamma_q = \set{\bind{x}{\modEntry{q.x}} \mid q.x \in \dom(\mathcal{M})} \runion \Delta$.}
\end{definition}
