...
\item a \emph{class entry} $C$, of the form \added{$\clsEntry{q.c}{\seq{x}}{C'}$}\deleted{$\clsEntryOld{\Gamma}{q}{\seq{x}}{c}$}, recording \added{the qualified name $q.c$ of the class, its}\deleted{the declaring context $\Gamma$, defining module $q$,} sequence of distinct field names $\seq{x}$, and \added{its base class entry $C'$}\deleted{base class $c$} (or $\bot$ for no base). \Added{A module declares each class name at most once, so a class entry is determined by its qualified name; the qualified name is the identity of the class.}
\end{itemize}
\end{definition}
...
\Added{A dataclass declaration counts as an assignment of its class name for these definitions, though a
class declaration re-using a name in scope is prohibited outright (\S\ref{sec:prohibited}).}
...
\item \Added{A class declaration re-using a name already in scope, including re-declaring a class name. Python permits the re-declaration, leaving the earlier class reachable only through its existing instances; \PurePy requires class names to be fresh, so that a qualified name identifies a class.}
