...
from contexts import (Context, ContextEntry, ModuleContext, ModuleLoaded, ModuleStub,
                      PREDEFINED_MEMBERS, PREDEFINED_MODULES, Status, Returns,
                      extend_context, predefined_context)
from aux import (annotate_seq_kinds, assigns_body, assigns_stmt, find_import, find_nested_import,
                 split_imports, statements)
...
def check_module_(m: ast.Module, M: dict[str, ast.Module], q: str) -> Context:
    ...
    returning = next((i for i in items if isinstance(result_type_of_statement(i), Returns)), None)
