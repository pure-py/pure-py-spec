...
Statement = Union[ast.stmt, list[ast.FunctionDef]]
...
def fv_stmt(s: ast.stmt) -> set[str]:
    ...
    if isinstance(s, ast.Match):
        return fv_e(s.subject) | set().union(*(fv_body(case.body) - binds(case.pattern) for case in s.cases))
...
def captures(s: ast.stmt) -> set[str]:
    ...
    if isinstance(s, ast.Match):
        return captures_e(s.subject) | set().union(*(captures_body(case.body) - binds(case.pattern) for case in s.cases))
    if isinstance(s, ast.FunctionDef):
...
def find_first_reassigning(items: list[Statement], names: set[str]) -> Optional[ast.AST]:
    if len(items) == 0:
        return None
