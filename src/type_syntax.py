...
class Literal:
    """Equality compares the value's Python type as well, since True == 1 and 1 == 1.0
...
def base_type(x: Literal | Type) -> Type:
    if isinstance(x, LiteralType):
        return base_type(x.ell)
...
def literal_type(e: ast.expr) -> LiteralType | None:
    ell = literal(e)
    return None if ell is None else LiteralType(ell)
